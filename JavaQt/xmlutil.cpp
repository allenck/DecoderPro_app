 ...
 */
/*public*/ /*static*/ void XMLUtil::write(QDomDocument doc, QTextStream* out, QString enc) //throws IOException
{
#if 0
    if (enc == "") {
 ...
 * @since 8.4
 */
/*public*/ /*static*/ QDomElement XMLUtil::findElement(QDomElement parent, QString name, QString _namespace) // throws IllegalArgumentException
{
 QDomElement result; // = null;
 QDomNodeList l = parent.childNodes();
 int nodeCount = l.size();
 for (int i = 0; i < nodeCount; i++)
 {
  ...
  if(l.at(i).isElement())
  {
   ...
   QString localName = node.localName();
   localName = localName == "" ? node.toElement().tagName() : localName;
   QString ns = node.namespaceURI();
   QString ns1 = node.toElement().attribute("xmlns");

   if (name == (localName)
           && (_namespace == "" || _namespace == (/*node.namespaceURI()*/ns1)))
   {
    if (result == QDomElement()) {
        result = /*(Element)*/ node.toElement();
    } else {
        Logger::error("more than one element with same name found");
    }
    break;
   }
  }
