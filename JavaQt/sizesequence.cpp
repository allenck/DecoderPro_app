...
/*private*/ /*static*/ QVector<int>* SizeSequence::emptyArray = new QVector<int>();
 ...
 * @see #setSizes(int[])
 */
/*public*/ SizeSequence::SizeSequence() {
    a = emptyArray;
}
 ...
 */
/*public*/ SizeSequence::SizeSequence(int numEntries)
{
 a = emptyArray;

    //this(numEntries, 0);
 insertEntries(0, numEntries, 0);
}
 ...
 */
/*public*/ SizeSequence::SizeSequence(int numEntries, int value) {
    //this();
 a = emptyArray;
 ...
 */
/*public*/ SizeSequence::SizeSequence(QVector<int>* sizes) {
    //this();
 a = emptyArray;
 ...
 */
void SizeSequence::setSizes(int length, int size) {
    if (a->length() != length) {
        a = new QVector<int>(length);
    }
    setSizes(0, length, size);
}

/*private*/ int SizeSequence::setSizes(int from, int to, int size) {
    if (to <= from) {
        return 0;
    }
    int m = (from + to)/2;
    a->replace(m, size + setSizes(from, m, size));
    return a->at(m) + setSizes(m + 1, to, size);
}
 ...
 */
/*public*/ void SizeSequence::setSizes(QVector<int>* sizes) {
    if (a->length() != sizes->length()) {
        a = new QVector<int>(sizes->length());
    }
    setSizes(0, a->length(), sizes);
}

/*private*/ int SizeSequence::setSizes(int from, int to, QVector<int>* sizes) {
    if (to <= from) {
        return 0;
    }
    int m = (from + to)/2;
    a->replace(m, sizes->at(m) + setSizes(from, m, sizes));
    return a->at(m) + setSizes(m + 1, to, sizes);
}
 ...
 */
/*public*/ QVector<int>* SizeSequence::getSizes() {
    int n = a->length();
    QVector<int>* sizes = new QVector<int>(n);
    getSizes(0, n, sizes);
    return sizes;
}

/*private*/ int SizeSequence::getSizes(int from, int to, QVector<int>* sizes) {
    if (to <= from) {
        return 0;
    }
    int m = (from + to)/2;
    sizes->replace(m, a->at(m) - getSizes(from, m, sizes));
    return a->at(m) + getSizes(m + 1, to, sizes);
}
 ...
 */
/*public*/ int SizeSequence::getPosition(int index) {
    return getPosition(0, a->length(), index);
}

/*private*/ int SizeSequence::getPosition(int from, int to, int index) {
    if (to <= from) {
        return 0;
    }
    int m = (from + to)/2;
    if (index <= m) {
        ...
        return a->at(m) + getPosition(m + 1, to, index);
    }
}
 ...
 */
/*public*/ int SizeSequence::getIndex(int position) {
    return getIndex(0, a->length(), position);
}

/*private*/ int SizeSequence::getIndex(int from, int to, int position) {
    if (to <= from) {
        return from;
    }
    int m = (from + to)/2;
 ...
 */
/*public*/ int SizeSequence::getSize(int index) {
 ...
 */
/*public*/ void SizeSequence::setSize(int index, int size) {
    changeSize(0, a->length(), index, size - getSize(index));
}

/*private*/ void SizeSequence::changeSize(int from, int to, int index, int delta) {
    if (to <= from) {
        return;
    }
    int m = (from + to)/2;
    if (index <= m) {
        a->replace(m, a->at(m)+ delta);
 ...
 */
/*public*/ void SizeSequence::insertEntries(int start, int length, int value) {
    QVector<int>* sizes = getSizes();
    int end = start + length;
    int n = a->length() + length;
    a = new QVector<int>(n);
    for (int i = 0; i < start; i++) {
        a->replace(i, sizes->at(i)) ;
    }
    for (int i = start; i < end; i++) {
        a->replace(i, value );
    }
    for (int i = end; i < n; i++) {
        a->replace(i, sizes->at(i-length)) ;
    }
    setSizes(a);
}
 ...
 */
/*public*/ void SizeSequence::removeEntries(int start, int length) {
    QVector<int>* sizes = getSizes();
    int end = start + length;
    int n = a->length() - length;
    a = new QVector<int>(n);
