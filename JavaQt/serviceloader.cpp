...
template<class S>
/*private*/ /*static*/ /*final*/ QString ServiceLoader<S>::PREFIX = "META-INF/services/";
 ...
 */
template<typename S>
/*public*/ void ServiceLoader<S>::reload() {
    providers->clear();
    lookupIterator = new LazyIterator(service, loader);
    QList<QString> lst = QList<QString>();
    if(service == "PreferencesManager")
    {
     lst
    ...
    // this is cheating, really need to look at the metadata
    if(service == "JsonServiceFactory")
    {
      lst << "JsonThrottleServiceFactory" << "JsonSensorServiceFactory" << "JsonPowerServiceFactory";
    }
    lookupIterator = QListIterator<QString>(lst);
}

template<typename S>
/*private*/ ServiceLoader<S>::ServiceLoader(QString svc, ClassLoader* cl) {
 providers = new OrderedMap<QString, S>();
...
//    String nextName = null;
    ...
    template<typename S>
    /*private*/ bool ServiceLoader<S>::LazyIterator::hasNextService() {
        if (nextName != "") {
            return true;
        }
#if 0
        if (configs == NULL) {
            try {
                QString fullName = PREFIX + service/*.getName()*/;
#if 0
                if (loader == NULL)
...
//            }
            ...
            //pending = parse(service, this->configs->nextElement());
            if(service == "PreferencesManager")
            {
             QList<QString> lst = QList<QString>()
              << "StartupActionsManager" << "GuiLafPreferencesManager" << "SystemConsolePreferencesManager"
                 << "FileLocationsPreferences" << "ManagerDefaultSelector" << "WarrantPreferences" << "RosterConfigManager"
                 << "ProgrammerConfigManager" << "ConnectionConfigManager" << "JmriJTablePersistenceManager";
             QListIterator<QString> iter = QListIterator<QString>(lst);
             pending = &iter;
            }
        }
    ...
    template<typename S>
    /*private*/ S ServiceLoader<S>::LazyIterator::nextService() {
        if (!hasNextService())
...
#else
        QObject* c = NULL;
        int typeId = QMetaType::type(cn.toLocal8Bit());
        if(typeId > 0)
        {
       #if QT_VERSION < 0x050000
         c = (QObject*)QMetaType::construct(typeId);
       #else
         c = (QObject*)QMetaType::create(typeId);
       #endif
        }
        S p = (S)c;
        providers->insert(cn, p);
        return p;
#endif
    }

    template<typename S>
    /*public*/ bool ServiceLoader<S>::LazyIterator::hasNext() {
        if (acc == NULL) {
    ...
    template<typename S>
    /*public*/ S ServiceLoader<S>::LazyIterator::next() {
        if (acc == NULL) {
            return nextService();
        }
#if 0
    ...
    template<typename S>
    /*public*/ void ServiceLoader<S>::LazyIterator::remove() {
 ...
 */
template<typename S>
/*public*/ Iterator<S> ServiceLoader<S>::iterator() {
 ...
 */
template<typename S>
/*public*/ /*static*/ ServiceLoader<S>* ServiceLoader<S>::load(QString service,
                                        ClassLoader* loader)
{
...
template<typename S>
/*public*/ /*static*/ ServiceLoader<S>* ServiceLoader<S>::load(QString service) {
    ClassLoader* cl = /*Thread.currentThread().getContextClassLoader();*/ new ClassLoader();
    return ServiceLoader<S>::load(service, cl);
}
#if 0
/**
 * Creates a new service loader for the given service type, using the
...
/*public*/ QString ServiceLoader<S>::toString() {
    return "java.util.ServiceLoader[" + service + "]";
}
