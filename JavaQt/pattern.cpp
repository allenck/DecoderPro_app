...
//{
   ...
   // private static final long serialVersionUID = 5073258162644648461L;
     ...
     */
    /*public*/ /*static*/ Pattern* Pattern::compile(QString regex) {
     ...
     *          If the expression's syntax is invalid
     */
    /*public*/ /*static*/ Pattern* Pattern::compile(QString regex, int flags) {
     ...
     */
    /*public*/ QString Pattern::pattern() {
        return _pattern;
    }
     ...
     * @since 1.5
     */
    /*public*/ QString Pattern::toString() {
        return _pattern;
    }
     ...
     *          If the expression's syntax is invalid
     */
    /*public*/ /*static*/ bool Pattern::matches(QString regex, CharSequence* input) {
     ...
     */
    /*public*/ QStringList* Pattern::split(CharSequence* input, int limit) {
        int index = 0;
        bool matchLimited = limit > 0;
        QStringList* matchList = new QStringList();
        ...
        while(m->find()) {
            if (!matchLimited || matchList->size() < limit - 1) {
                ...
                QString match = input->subSequence(index, m->start())->toString();
                matchList->append(match);
                index = m->end();
            } else if (matchList->size() == limit - 1) { // last one
                QString match = input->subSequence(index,
        ...
        if (index == 0)
            return new QStringList {input->toString()};

        // Add remaining segment
        if (!matchLimited || matchList->size() < limit)
            matchList->append(input->subSequence(index, input->length())->toString());

        // Construct result
        int resultSize = matchList->size();
        if (limit == 0)
            while (resultSize > 0 && matchList->at(resultSize-1)==(""))
                resultSize--;
        QStringList result =  ( QVector<QString>(resultSize)).toList();
        return matchList->mid(0, resultSize).toArray(result);
    }
#if 0
    /**
     * Splits the given input sequence around matches of this pattern.
     ...
     */
    /*private*/ Pattern::Pattern(QString p, int f) {
        _pattern = p;
        _flags = f;
     ...
     */
    /*private*/ void Pattern::normalize() {
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    /*private*/ void Pattern::compile() {
        ...
        temp = new QVector<int>(patternLength + 2);
...
//        bool deterministic;
     ...
     */
    /*private*/ bool Pattern::has(int f) {
        return (flags & f) != 0;
    }
     ...
     */
    /*private*/ void Pattern::accept(int ch, QString s) {
     ...
     */
    /*private*/ void Pattern::mark(int c) {
     ...
     */
    /*private*/ int Pattern::readEscaped() {
        int ch = temp[cursor++];
        return ch;
    }
     ...
     * ignoring the COMMENTS setting
     */
    /*private*/ int Pattern::nextEscaped() {
        int ch = temp[++cursor];
        return ch;
    }
     ...
     */
    /*private*/ int Pattern::peekPastWhitespace(int ch) {
     ...
     */
    /*private*/ int Pattern::parsePastWhitespace(int ch) {
        while (ASCII.isSpace(ch) || ch == '#') {
            while (ASCII.isSpace(ch))
                ch = temp[cursor++];
            if (ch == '#')
     ...
     */
    /*private*/ int Pattern::parsePastLine() {
        int ch = temp[cursor++];
     ...
     */
    /*private*/ int Pattern::peekPastLine() {
        int ch = temp[++cursor];
        while (ch != 0 && !isLineSeparator(ch))
            ch = temp[++cursor];
        return ch;
     ...
     */
    /*private*/ bool Pattern::isLineSeparator(int ch) {
        if (has(UNIX_LINES)) {
            return ch == '\n';
        } else {
            return (ch == '\n' ||
     ...
     */
    /*private*/ PatternSyntaxException Pattern::error(QString s) {
     ...
     */
    /*private*/ bool Pattern::findSupplementary(int start, int end) {
     ...
     */
    /*private*/ /*static*/ /*final*/ bool Pattern::isSupplementary(int ch) {
     ...
     */
    /*private*/ Node Pattern::expr(Node end) {
        Node* prev = NULL;
        Node* firstTail = NULL;
        Branch* branch = NULL;
    ...
    /*private*/ void Pattern::append(int ch, int len) {
     ...
     */
    /*private*/ Node* Pattern::ref(int refNum) {
        bool done = false;
        while(!done) {
            int ch = peek();
            switch(ch) {
...
//        Node next;
         ...
         */
        bool Node::match(Matcher* matcher, int i, CharSequence* seq) {
            matcher->last = i;
...
//    static class LastNode extends Node {
         ...
         */
        bool LastNode::match(Matcher* matcher, int i, CharSequence* seq) {
...
//        int minLength;
        ...
        bool Start::match(Matcher* matcher, int i, CharSequence* seq) {
        ...
        bool Start::study(TreeInfo* info) {
            next->study(info);
            info->maxValid = false;
...
//        int localIndex;
        GroupHead::GroupHead(int localCount) {
            localIndex = localCount;
        }
        bool GroupHead::match(Matcher* matcher, int i, CharSequence* seq) {
            int save = matcher->locals.at(localIndex);
            matcher->locals[localIndex] = i;
            bool ret = next.match(matcher, i, seq);
            matcher->locals[localIndex] = save;
            return ret;
        }
        bool GroupHead::matchRef(Matcher* matcher, int i, CharSequence* seq) {
...
///////////////////////////////////////////////////////////////////////////////
     ...
     */
    /*static*/ Node* Pattern::_accept = new Node();

    /*static*/ Node* Pattern::_lastAccept = new LastNode();
#if 0
