...
//JEditorPane::JEditorPane(QWidget *parent) :
 ...
 */
/*public*/ JEditorPane::JEditorPane(QWidget *parent) :
  JTextComponent(parent)
{
    //super();
 common();
}
void JEditorPane::common()
{
#if 0
 ...
 */
/*public*/ JEditorPane::JEditorPane(QUrl* initialPage, QWidget *parent) :
  JTextComponent(parent)  /*throws IOException*/
{
    //this();
 common();
#if 0
 ...
 *          cannot be accessed
 */
/*public*/ JEditorPane::JEditorPane(QString url, QWidget *parent) :
  JTextComponent(parent)  /*throws IOException*/
{
 //this();
 common();
 ...
 *          is <code>null</code>
 */
/*public*/ JEditorPane::JEditorPane(QString type, QString text, QWidget *parent) :
  JTextComponent(parent)
{
    //this();
 common();
...
/*public*/ void setPage(URL page) throws IOException {
    ...
    final QString reference = page.getRef();
    if (reference != null) {
        if (!reloaded) {
...
/*public*/ void read(InputStream in, Object desc) throws IOException {
    ...
    if (desc instanceof HTMLDocument &&
        ...
        QString charset = (QString) getClientProperty("charset");
...
void read(InputStream in, Document doc) throws IOException {
    ...
    try {
        QString charset = (String) getClientProperty("charset");
        Reader r = (charset != null) ? new InputStreamReader(in, charset) :
            new InputStreamReader(in);
        kit.read(r, doc, 0);
...
protected InputStream getStream(URL page) throws IOException {
    ...
    if (conn instanceof HttpURLConnection) {
         ...
         */
        if (redirect) {
            QString loc = conn.getHeaderField("Location");
...
private void handleConnectionProperties(URLConnection conn) {
    if (pageProperties == null) {
        pageProperties = new Hashtable<QString, Object>();
    }
    QString type = conn.getContentType();
    if (type != null) {
        setContentType(type);
        pageProperties.put("content-type", type);
    }
    pageProperties.put(Document.StreamDescriptionProperty, conn.getURL());
    QString enc = conn.getContentEncoding();
    if (enc != null) {
 ...
 */
/*public*/ void scrollToReference(QString reference) {
    ...
    if (d instanceof HTMLDocument) {
        ...
        for (; iter.isValid(); iter.next()) {
            ...
            QString nm = (String) a.getAttribute(HTML.Attribute.NAME);
 ...
 *          specification
 */
/*public*/ void setPage(QString url) throws IOException {
    if (url == null) {
        throw new IOException("invalid url");
    }
 ...
 */
/*public*/ final QString getContentType() {
 ...
 *          is <code>null</code>
 */
/*public*/ final void setContentType(QString type) {
    ...
    if (parm > -1) {
        // Save the paramList.
        QString paramList = type.substring(parm);
 ...
 */
private void setCharsetFromContentTypeParameters(QString paramlist) {
    QString charset;
    try {
 ...
 * @return the editor kit
 */
/*public*/ EditorKit getEditorKitForContentType(QString type) {
    if (typeHandlers == null) {
        typeHandlers = new Hashtable<QString, EditorKit>(3);
    }
 ...
 */
/*public*/ void setEditorKitForContentType(QString type, EditorKit k) {
    if (typeHandlers == null) {
        typeHandlers = new Hashtable<QString, EditorKit>(3);
    }
 ...
 */
@Override
/*public*/ void replaceSelection(QString content) {
    if (! isEditable()) {
        UIManager.getLookAndFeel().provideErrorFeedback(JEditorPane.this);
        return;
    }
        ...
        try {
            ...
            if (doc instanceof AbstractDocument) {
                ...
                if (content != null && content.length() > 0) {
                    doc.insertQString(p0, content, ((StyledEditorKit)kit).
                                     getInputAttributes());
                }
            }
 ...
 */
/*public*/ static EditorKit createEditorKitForContentType(QString type) {
    Hashtable<QString, EditorKit> kitRegistry = getKitRegisty();
    EditorKit k = kitRegistry.get(type);
    if (k == null) {
        // try to dynamically load the support
        QString classname = getKitTypeRegistry().get(type);
 ...
 */
/*public*/ static void registerEditorKitForContentType(QString type, QString classname) {
 ...
 */
/*public*/ static void registerEditorKitForContentType(QString type, QString classname, ClassLoader loader) {
 ...
 * @since 1.3
 */
/*public*/ static QString getEditorKitClassNameForContentType(QString type) {
    return getKitTypeRegistry().get(type);
}

private static Hashtable<QString, QString> getKitTypeRegistry() {
    loadDefaultKitsIfNecessary();
    return (Hashtable)SwingUtilities.appContextGet(kitTypeRegistryKey);
}

private static Hashtable<QString, ClassLoader> getKitLoaderRegistry() {
    loadDefaultKitsIfNecessary();
    return (Hashtable)SwingUtilities.appContextGet(kitLoaderRegistryKey);
}

private static Hashtable<QString, EditorKit> getKitRegisty() {
...
private static void loadDefaultKitsIfNecessary() {
    if (SwingUtilities.appContextGet(kitTypeRegistryKey) == null) {
        ...
        for (QString key : defaultEditorKitMap.keySet()) {
...
private Hashtable<QString, Object> pageProperties;

/** Should be kept in sync with javax.swing.text.html.FormView counterpart. */
final static QString PostDataProperty = "javax.swing.JEditorPane.postdata";

/**
 * Table of registered type handlers for this editor.
 */
private Hashtable<QString, EditorKit> typeHandlers;
 ...
 * @see #readObject
 */
private static final QString uiClassID = "EditorPaneUI";
 ...
 * @since 1.5
 */
/*public*/ static final QString W3C_LENGTH_UNITS = "JEditorPane.w3cLengthUnits";
 ...
 * @since 1.5
 */
/*public*/ static final QString HONOR_DISPLAY_PROPERTIES = "JEditorPane.honorDisplayProperties";

static final Map<QString, QString> defaultEditorKitMap = new HashMap<QString, QString>(0);
...
protected QString paramString() {
    QString kitString = (kit != null ?
                        kit.toString() : "");
    QString typeHandlersString = (typeHandlers != null ?
...
protected class AccessibleJEditorPane extends AccessibleJTextComponent {
     ...
     */
    /*public*/ QString getAccessibleDescription() {
        QString description = accessibleDescription;

        // fallback to client property
        if (description == null) {
            description = (QString)getClientProperty(AccessibleContext.ACCESSIBLE_DESCRIPTION_PROPERTY);
        }
        if (description == null) {
...
extends AccessibleJEditorPane implements AccessibleHypertext {
    ...
    /*public*/ class HTMLLink extends AccessibleHyperlink {
         ...
         * @see #getAccessibleActionCount
         */
        /*public*/ QString getAccessibleActionDescription(int i) {
            if (i == 0 && isValid() == true) {
    ...
    private void buildLinkTable() {
        ...
        if (d != null) {
            ...
            while ((e = ei.next()) != null) {
                ...
                href = (anchor != null) ?
                    (QString) anchor.getAttribute(HTML.Attribute.HREF) : null;
                    if (href != null) {
     ...
     */
    /*public*/ QString getLinkText(int linkIndex) {
        if (linksValid == false) {
            buildLinkTable();
        }
...
/*public*/ HeaderParser(QString raw) {
    this.raw = raw;
    tab = new String[10][2];
    parse();
}
...
/*public*/ QString findKey(int i) {
    if (i < 0 || i > 10)
        return null;
    return tab[i][0];
}

/*public*/ QString findValue(int i) {
    if (i < 0 || i > 10)
        return null;
    return tab[i][1];
}

/*public*/ QString findValue(QString key) {
    return findValue(key, null);
}

/*public*/ QString findValue(QString k, QString Default) {
    if (k == null)
        return Default;
    k = k.toLowerCase();
    for (int i = 0; i < 10; ++i) {
...
/*public*/ int findInt(QString k, int Default) {
    try {
