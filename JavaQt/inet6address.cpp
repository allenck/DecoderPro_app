...
//class Inet6Address extends InetAddress {
    /*final*/ /*static*/ int Inet6Address::INADDRSZ = 16;


//    /*private*/ class Inet6AddressHolder {

        /*private*/ Inet6AddressHolder::Inet6AddressHolder() {
            ipaddress = QByteArray( Inet6Address::INADDRSZ,0);
        }

        /*private*/ Inet6AddressHolder::Inet6AddressHolder(
            QByteArray ipaddress, int scope_id, bool scope_id_set,
        ...
        void Inet6AddressHolder::setAddr(QByteArray addr) {
            if (addr.length() == Inet6Address::INADDRSZ) { // normal IPv6 address
                //System.arraycopy(addr, 0, ipaddress, 0, Inet6Address::INADDRSZ);
             addr = QByteArray(ipaddress);
            }
        }
        ...
        QString Inet6AddressHolder::getHostAddress() {
            QString s = Inet6Address::numericToTextFormat(ipaddress);
            if (!scope_ifname.isValid()) { /* must check this first */
        ...
        /*public*/ bool Inet6AddressHolder::equals(QObject* o) {
            if (! (qobject_cast<Inet6AddressHolder*>(o))) {
                return false;
            }
        ...
        /*public*/ int Inet6AddressHolder::hashCode() {
        ...
        bool Inet6AddressHolder::isIPv4CompatibleAddress() {
        ...
        bool Inet6AddressHolder::isMulticastAddress() {
            return ((ipaddress[0] & 0xff) == 0xff);
        }

        bool Inet6AddressHolder::isAnyLocalAddress() {
            char test = 0x00;
            for (int i = 0; i < Inet6Address::INADDRSZ; i++) {
                test |= ipaddress[i];
            }
            return (test == 0x00);
        }

        bool Inet6AddressHolder::isLoopbackAddress() {
            char test = 0x00;
            for (int i = 0; i < 15; i++) {
                test |= ipaddress[i];
            }
        ...
        bool Inet6AddressHolder::isLinkLocalAddress() {
            return ((ipaddress[0] & 0xff) == 0xfe
                    && (ipaddress[1] & 0xc0) == 0x80);
        }


        bool Inet6AddressHolder::isSiteLocalAddress() {
            return ((ipaddress[0] & 0xff) == 0xfe
                    && (ipaddress[1] & 0xc0) == 0xc0);
        }

        bool Inet6AddressHolder::isMCGlobal() {
            return ((ipaddress[0] & 0xff) == 0xff
                    && (ipaddress[1] & 0x0f) == 0x0e);
        }

        bool Inet6AddressHolder::isMCNodeLocal() {
            return ((ipaddress[0] & 0xff) == 0xff
                    && (ipaddress[1] & 0x0f) == 0x01);
        }

        bool Inet6AddressHolder::isMCLinkLocal() {
            return ((ipaddress[0] & 0xff) == 0xff
                    && (ipaddress[1] & 0x0f) == 0x02);
        }

        bool Inet6AddressHolder::isMCSiteLocal() {
            return ((ipaddress[0] & 0xff) == 0xff
                    && (ipaddress[1] & 0x0f) == 0x05);
        }

        bool Inet6AddressHolder::isMCOrgLocal() {
            return ((ipaddress[0] & 0xff) == 0xff
...
//    /*private*/ static /*final*/ long serialVersionUID = 6880410070516793377L;
     ...
     */
    Inet6Address::Inet6Address(QString hostName, QByteArray addr, int scope_id) {
        _holder->init(hostName, IPv6);
        holder6 = new Inet6AddressHolder();
...
#if 0
    Inet6Address (QString hostName, Q, NetworkInterface nif)
...
#endif
    // Utilities
    /*private*/ /*final*/ /*static*/ int Inet6Address::INT16SZ = 2;
     ...
     * @since 1.4
     */
    /*static*/ QString Inet6Address::numericToTextFormat(QByteArray src) {
        QString sb ;//= new StringBuilder(39);
        for (int i = 0; i < (INADDRSZ / INT16SZ); i++) {
            sb.append(QString::number(((src[i<<1]<<8) & 0xff00)
                                          | (src[(i<<1)+1] & 0xff),16));
