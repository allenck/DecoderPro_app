...
ColorModel::ColorModel(QString name, QStringList labels, QObject *parent) : QObject(parent) {
...
ColorModel::ColorModel(QObject *parent) : QObject(parent)
 ...
 labels = QStringList();
 labels << "rgb"<< "Red"<< "Green"<< "Blue"<< "Alpha";
}

void ColorModel::setColor(int color, QVector<float> model) {
...
int ColorModel::getColor(QVector<float> model) {
    return to8bit(model[2]) | (to8bit(model[1]) << 8) | (to8bit(model[0]) << 16) | (to8bit(model[3]) << 24);
}

int ColorModel::getCount() {
    return this->labels.length();
}

int ColorModel::getMinimum(int index) {
    return 0;
}

int ColorModel::getMaximum(int index) {
    return 255;
}

float ColorModel::getDefault(int index) {
    return 0.0f;
}

/*final*/ QString ColorModel::getLabel(Component* component, int index) {
    return getText(component, this->labels[index]);
}

/*private*/ /*static*/ float ColorModel::normalize(int value) {
    return (float) (value & 0xFF) / 255.0f;
}

/*private*/ /*static*/ int ColorModel::to8bit(float value) {
