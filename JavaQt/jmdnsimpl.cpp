...
// /*public*/ class JmDNSImpl extends JmDNS implements DNSStatefulObject, DNSTaskStarter {
    /*private*/ /*static*/ Logger* JmDNSImpl::logger = LoggerFactory::getLogger("JmDNSImpl");
...
//    /*public*/ static class ServiceTypeEntry extends AbstractMap<String, String>implements Cloneable {
...
#endif
        _services = QMap<QString, ServiceInfo*>(/*20*/);
#if 0
        _serviceTypes = new ConcurrentHashMap<String, ServiceTypeEntry>(20);
#endif
        _name = (name != "" ? name : QHostInfo::localHostName());//_localHost.getName());

        _localHost = QHostInfo::fromName(_name);//HostInfo.newHostInfo(address, this, name);
     ...
     */
    //@Override
    /*public*/ bool JmDNSImpl::isClosing() {
 ...
 return false;
     ...
     */
    //@Override
    /*public*/ bool JmDNSImpl::isClosed() {
//        return this->_localHost.isClosed();
     QList<QHostAddress> addresses = this->_localHost.addresses();
     ...
     */
    //@Override
    /*public*/ QString JmDNSImpl::getName() {
        return _name;
    }
     ...
     */
    //@Override
    /*public*/ QString JmDNSImpl::getHostName() {
        return QHostInfo::localHostName();//_localHost.getName();
    }
     ...
     */
    /*public*/ QHostInfo JmDNSImpl::getLocalHost() {
        return _localHost;
    }
...
//        return _localHost.getInetAddress();
    ...
    /*public*/ void JmDNSImpl::registerService(ServiceInfo* infoAbstract) /*throw (IOException)*/ {
        ...
        info->recoverState();
#endif
        info->setServer(_localHost.hostName());
        //info->addAddress(_localHost.getInet4Address());
        QList<QHostAddress> addresses = _localHost.addresses();
        foreach (QHostAddress addr, addresses) {
         if(addr.protocol()== QAbstractSocket::IPv4Protocol)
         {

          info->addAddress(new Inet4Address(_localHost.hostName(), addr.toIPv4Address()));
         }
        }
        ...
        foreach (QHostAddress addr, addresses) {
         ...
         {
          Q_IPV6ADDR bytes = addr.toIPv6Address();
          QByteArray ba = QByteArray();
          for(int i=0; i < 16; i ++)
           ba.append(bytes[i]);
          info->addAddress(new Inet6Address(_localHost.hostName(), ba));
         }
        }
#if 0
        this->makeServiceNameUnique(info);
        while (_services.putIfAbsent(info->getKey(), info) != nullptr) {
            this->makeServiceNameUnique(info);
        }

        this->startProber();
#endif
        logger->debug(tr("registerService() JmDNS registered service as %1").arg(info->toString()));
    }
    ...
    /*public*/ void JmDNSImpl::unregisterService(ServiceInfo* infoAbstract) {
     ...
     Q_UNUSED(info)
        ...
        if (info != nullptr) {
            ...
            _services.remove(info->key(), info);
            logger->debug(tr("unregisterService() JmDNS %1 unregistered service as %2").arg(this->getName()).arg(info));
        } else {
            logger->warn(tr("%1 removing unregistered service info: %2").arg(this->getName()).arg( infoAbstract->getKey()));
        }
#endif
    }
#if 0
     ...
     */
    //@Override
    /*public*/ void JmDNSImpl::close() {
        if (this->isClosing()) {
            return;
        }

        logger->debug(tr("Cancelling JmDNS: %1").arg(this->getName()));
#if 0
        // Stop JmDNS
        // This protects against recursive calls
        if (this->cancelState()) {
