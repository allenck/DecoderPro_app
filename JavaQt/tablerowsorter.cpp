...
#include "tablestringconverter.h"
 ...
 */
/*private*/ /*static*/ /*final*/ Comparator* TableRowSorter::COMPARABLE_COMPARATOR =
 ...
 */
/*public*/ TableRowSorter::TableRowSorter() {
    //this(NULL);
 setModel(NULL);
}
 ...
 */
/*public*/ void TableRowSorter::setStringConverter(TableStringConverter* stringConverter) {
 ...
 */
/*public*/ TableStringConverter* TableRowSorter::getStringConverter() {
 ...
 */
/*public*/ Comparator/*<?>*/* TableRowSorter::getComparator(int column) {
    Comparator* comparator = DefaultRowSorter::getComparator(column);
    if (comparator != NULL) {
        return comparator;
    }
#if 0
 ...
 * @throws IndexOutOfBoundsException {@inheritDoc}
 */
/*protected*/ bool TableRowSorter::useToString(int column) {
    Comparator* comparator = DefaultRowSorter::getComparator(column);
    if (comparator != NULL) {
        return false;
    }
...
///*private*/ class TableRowSorterModelWrapper extends ModelWrapper<M,Integer> {
    TableRowSorterModelWrapper::TableRowSorterModelWrapper(TableRowSorter *trs) { this->trs = trs;}
    /*public*/ QAbstractItemModel* TableRowSorterModelWrapper::getModel() {
        return trs->tableModel;
    }

    /*public*/ int TableRowSorterModelWrapper::getColumnCount() {
        return (trs->tableModel == NULL) ? 0 : trs->tableModel->/*getColumnCount()*/columnCount();
    }

    /*public*/ int TableRowSorterModelWrapper::getRowCount() {
        return (trs->tableModel == NULL) ? 0 : trs->tableModel->/*getRowCount()*/rowCount();
    }

    /*public*/ QVariant TableRowSorterModelWrapper::getValueAt(int row, int column) {
        return trs->tableModel->/*getValueAt(row, column)*/data(trs->tableModel->index(row,column));
    }

    /*public*/ QString TableRowSorterModelWrapper::getStringValueAt(int row, int column) {
        TableStringConverter* converter = trs->getStringConverter();
        if (converter != NULL) {
            // Use the converter
            QString value = converter->toString(trs->tableModel, row, column);
            if (value != NULL) {
                return value;
            }
            return "";
    ...
    /*public*/ int TableRowSorterModelWrapper::getIdentifier(int index) {
        return index;
    }
//};
...
//    @SuppressWarnings("unchecked")
    /*public*/ int ComparableComparator::compare(QObject* o1, QObject* o2) {
        //return ((Comparable<QVariant>*)o1)->compareTo(o2);
     return ((Comparable<QObject*>*)o1)->compareTo(o2);
    }
//};
