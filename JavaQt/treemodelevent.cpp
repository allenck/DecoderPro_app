 ...
 * @see TreePath
 */
/*public*/ TreeModelEvent::TreeModelEvent(QObject* source, QVector<QObject*>* path, QVector<int>* childIndices, QVector<QObject*>* children, QObject* parent) : EventObject(source, parent)
{
    common( (path == NULL) ? NULL : new TreePath(path), childIndices, children);
}
 ...
 */
/*public*/ TreeModelEvent::TreeModelEvent(QObject* source, TreePath* path, QVector<int>* childIndices, QVector<QObject*>* children, QObject* parent) : EventObject(source, parent)
{
    //super(source);
    common(path, childIndices, children);
}

void TreeModelEvent::common( TreePath* path, QVector<int>* childIndices, QVector<QObject*>* children)
{
 this->path = path;
 ...
 * @see TreePath
 */
/*public*/ TreeModelEvent::TreeModelEvent(QObject* source, QVector<QObject*>* path, QObject* parent) : EventObject(source, parent)
{
    common((path == NULL) ? NULL : new TreePath(path), NULL, NULL);
}
 ...
 */
/*public*/ TreeModelEvent::TreeModelEvent(QObject* source, TreePath* path, QObject* parent) : EventObject(source, parent)
{
    //super(source);
    this->path = path;
    this->childIndices = new QVector<int>(); //[0];
}
 ...
 */
/*public*/ TreePath* TreeModelEvent::getTreePath() { return path; }
 ...
 * @see #getChildIndices
 */
/*public*/ QList<QObject*>* TreeModelEvent::getChildren() {
    if(children != NULL) {
        int            cCount = children->length();
        QObjectList*       retChildren = new QObjectList();//Object[cCount];

        //System.arraycopy(children, 0, retChildren, 0, cCount);
        foreach (QObject* obj, *children) {
         retChildren->append(obj);
        }
        return retChildren;
    }
    return NULL;
 ...
 */
/*public*/ QVector<int>* TreeModelEvent::getChildIndices() {
    if(childIndices != NULL) {
        int            cCount = childIndices->length();
        QVector<int>*         retArray = new QVector<int>(cCount);

        //System.arraycopy(childIndices, 0, retArray, 0, cCount);
        for(int i = 0; i < cCount; i++)
         retArray->replace(i, childIndices->at(i));
        return retArray;
    }
    return NULL;
}
 ...
 */
/*public*/ QString TreeModelEvent::toString() {
    QString   retBuffer; // = new StringBuffer();

    retBuffer.append(QString(metaObject()->className()) + " " /*+
                     Integer.toString(hashCode())*/);
    if(path != NULL)
        retBuffer.append(" path " + path->toString());
    if(childIndices != NULL) {
        retBuffer.append(" indices [ ");
        for(int counter = 0; counter < childIndices->length(); counter++)
            retBuffer.append(QString::number(childIndices->at(counter))+ " ");
        retBuffer.append("]");
    }
    if(children != NULL) {
        retBuffer.append(" children [ ");
        for(int counter = 0; counter < children->length(); counter++)
            retBuffer.append(QString(children->at(counter)->metaObject()->className()) + " ");
        retBuffer.append("]");
    }
