...
/*private*/ /*static*/ int BufferedWriter::defaultCharBufferSize;// = 8192;
     ...
     * @param  out  A Writer
     */
    /*public*/ BufferedWriter::BufferedWriter(Writer* out) : Writer(){
        common(out, defaultCharBufferSize);
    }
     ...
     */
    /*public*/ BufferedWriter::BufferedWriter(Writer* out, int sz) : Writer()
    {
        //super(out);
        common(out, sz);
    }

    void BufferedWriter::common(Writer* out, int sz)
    {
        if (sz <= 0)
     ...
     */
    /*private*/ int BufferedWriter::min(int a, int b) {
        if (a < b) return a;
        return b;
    }
    ...
    /*public*/ void BufferedWriter::write(QByteArray cbuf, int off, int len) /*throw (IOException)*/ {
        /*synchronized (lock)*/ {
            ...
            while (b < t) {
                ...
                cbuf = QByteArray((QByteArray)cbuf.mid(0,b)+ cb.mid(nextChar,d));
                b += d;
                nextChar += d;
                if (nextChar >= nChars)
                    flushBuffer();
