...
//@Nonnull
template<class E>
/*abstract*/ class AbstractProvidingProxyManager : public AbstractProxyManager<E> implements ProvidingManager<E> {
  /**
   * Locate via user name, then system name if needed. If that fails, create a
   * new NamedBean: If the name is a valid system name, it will be used for
   * the new NamedBean. Otherwise, the makeSystemName method will attempt to
   ...
   */
  //@Nonnull
  /*protected*/ E provideNamedBean(QString name) throw (IllegalArgumentException) {
 ...
 */
//@Nonnull
/*public*/ E newNamedBean(/*@Nonnull*/ QString systemName, QString userName) throw (IllegalArgumentException) {
    // make sure internal present
    initInternal();
