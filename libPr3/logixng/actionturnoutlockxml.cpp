...
ActionTurnoutLockXml::ActionTurnoutLockXml(QObject *parent)
    : AbstractNamedBeanManagerConfigXML(parent)
{
     ...
     */
    //@Override
    /*public*/ QDomElement ActionTurnoutLockXml::store(QObject* o) {
        ActionTurnoutLock* p = (ActionTurnoutLock*) o;

        QDomElement element = doc.createElement("ActionTurnoutLock");
        QDomElement e1;
        element.setAttribute("class", /*this.getClass().getName()*/"jmri.jmrit.logixng.actions.configurexml.ActionTurnoutLockXml");
        ...
        element.appendChild(e1=doc.createElement("stateAddressing")); e1.appendChild(doc.createTextNode(NamedBeanAddressing::toString(p->getLockAddressing())));
        element.appendChild(e1=doc.createElement("turnoutLock")); e1.appendChild(doc.createTextNode(ActionTurnoutLock::TurnoutLock::toString(p->getTurnoutLock())));
        element.appendChild(e1=doc.createElement("stateReference")); e1.appendChild(doc.createTextNode(p->getLockReference()));
        element.appendChild(e1=doc.createElement("stateLocalVariable")); e1.appendChild(doc.createTextNode(p->getLockLocalVariable()));
        element.appendChild(e1=doc.createElement("stateFormula")); e1.appendChild(doc.createTextNode(p->getLockFormula()));

        return element;
    }

    //@Override
    /*public*/ bool ActionTurnoutLockXml::load(QDomElement shared, QDomElement perNode) /*throws JmriConfigureXmlException*/ {     // Test class that inherits this class throws exception
        ...
        try {
            ...
            if (!elem.isNull()) {
                h->setLockAddressing(NamedBeanAddressing::valueOf(elem.text().trimmed()));
            }

            QDomElement turnoutLock = shared.firstChildElement("turnoutLock");
            if (!turnoutLock.isNull()) {
                h->setTurnoutLock(ActionTurnoutLock::TurnoutLock::valueOf(turnoutLock.text().trimmed()));
            }

            elem = shared.firstChildElement("stateReference");
            if (!elem.isNull()) h->setLockReference(elem.text().trimmed());

            elem = shared.firstChildElement("stateLocalVariable");
            if (!elem.isNull()) h->setLockLocalVariable(elem.text().trimmed());

            elem = shared.firstChildElement("stateFormula");
            if (!elem.isNull()) h->setLockFormula(elem.text().trimmed());
