...
//         implements FemaleSocketListener {
    ...
    /*public*/  void Logix::disconnected(FemaleSocket* socket) {
        if (socket == _expressionSocket) {
            ...
            throw new IllegalArgumentException("Logix: unknown socket");
        }
    }
    ...
    /*public*/  void Logix::setup() {
        try {
            ...
            if ( !_actionSocket->isConnected()
                ...
                if (socketSystemName != "") {
                    MaleSocket* maleSocket = (AbstractMaleSocket*)((DefaultDigitalBooleanActionManager*)
                    ...
                    if (maleSocket != nullptr) {
                        ...
                        log->error("cannot load digital bool action " + socketSystemName);
                    }
                }
            } else {
    ...
    /*private*/ /*final*/ /*static*/ Logger* log = LoggerFactory::getLogger("Logix");
}
