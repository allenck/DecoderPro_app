    ...
    /*synchronized*/ /*protected*/ void waitForStartblockToGetOccupied() {
        while (!isStartBlockOccupied()) {
            ...
            try {
                ...
                log->debug(tr("%1 waitForStartblockToGetOccupied InterruptedException %2").arg(_trainName),ie,ie);
            }
            catch(Exception e){
                log->debug(tr("%1 waitForStartblockToGetOccupied unexpected exception %2",_trainName),e,e);
            }
        }
    }
#endif
