 ...
 */
/*public*/ /*static*/ /*final*/ int AudioSystem::NOT_SPECIFIED = -1;

/**
 * Private no-args constructor for ensuring against instantiation.
 */
/*private*/ AudioSystem::AudioSystem(QObject *parent)  : QObject(parent)
{
...
#if 0
 //List codecs = getFormatConversionProviders();
 QStringList codecs = QAudioDeviceInfo::supportedCodecs();
 ...
 * @see InputStream#mark
 */
/*public*/ /*static*/ AudioInputStream* AudioSystem::getAudioInputStream(QDataStream* stream)
    /*throws UnsupportedAudioFileException, IOException*/
{
#if 1
 Logger* log = new Logger("AudioSystem");
 QList<AudioFileReader*>* providers = getAudioFileReaders();
 AudioInputStream* audioStream = NULL;

 for(int i = 0; i < providers->size(); i++ )
 {
  AudioFileReader* reader = (AudioFileReader*) providers->at(i);
//  try
//  {
  if(qobject_cast<WavFileReader*>(reader))
   audioStream = ((WavFileReader*)reader)->getAudioInputStream( stream );
  else
...
//  }
 ...
 if( audioStream==NULL )
 {
  ...
  log->error("could not get audio input stream from input stream");
 }
 else
 {
  return audioStream;
 ...
 * @throws IOException if an I/O exception occurs
 */
/*public*/ /*static*/ AudioInputStream* AudioSystem::getAudioInputStream(File* file)
    /*throws UnsupportedAudioFileException, IOException */
{
 QList<AudioFileReader*>* providers = getAudioFileReaders();
 AudioInputStream* audioStream = NULL;
...
//  }
 ...
 if( audioStream==NULL )
 {
  ...
  Logger::error("could not get audio input stream from input file " + file->getPath());
 }
 else
 {
  return audioStream;
 ...
 */
/*private*/ /*static*/ QList<AudioFileReader*>* AudioSystem::getAudioFileReaders()
{
 //return getProviders(AudioFileReader.class);
 QList<AudioFileReader*>* ret = new QList<AudioFileReader*>();
 ret->append(new WavFileReader());
