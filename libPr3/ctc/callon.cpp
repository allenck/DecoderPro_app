...
//  NOTE: When calling this constructor, ALL values MUST BE VALID!  NO check is done!
    /*public*/ GroupingData(NBHSignal* signal, QString signalHeadFaces, int callOnAspect, NBHSensor* calledOnExternalSensor, NamedBeanHandle<Block*>* namedBeanHandleBlock, SwitchIndicatorsRoute* route) {
        _mSignal = signal;
        _mSignalHeadFaces = signalHeadFaces == (QString("LEFTTRAFFIC")) ? CTCConstants::LEFTTRAFFIC : CTCConstants::RIGHTTRAFFIC;   // NOI18N
...
/*public*/ CallOn::CallOn(LockedRoutesManager* lockedRoutesManager, QString userIdentifier, NBHSensor* callOnToggleSensor, QList<CallOnData*> groupingsList, OtherData::SIGNAL_SYSTEM_TYPE signalSystemType, QObject* parent) : QObject(parent) {
...
//  Technically, I'd have liked to call this only once, but in reality, each signalhead could have a different value list:
                QStringList validStateNames = signal->getValidStateNames().toList(); // TODO consider using getValidStateKeys() to skip localisation issue
                ...
                _mGroupingDataArrayList.append(new GroupingData(signal, trafficDirection, correspondingValidStates[validStateNamesIndex], calledOnExternalSensor, nullptr, route));
            } else {
                NamedBeanHandle<Block*>* externalBlock = callOnData->_mExternalBlock;
                _mGroupingDataArrayList.append(new GroupingData(signal, trafficDirection, 0, nullptr, externalBlock, route));
            }
...
/*public*/ void CallOn::removeAllListeners() {}   // None done.

/*public*/ void CallOn::resetToggle() {
...
*/
/*public*/ TrafficLockingInfo* CallOn::codeButtonPressed(QSet<Sensor*> sensors,
    ...
    GroupingData* foundGroupingData = nullptr;
...
//  From NOW ON, the "returnValue" status will be true:
    ...
    if (foundGroupingData == nullptr) return returnValue;    // Has to be active, pretend we did it, but we didn't!
    ...
    if (_mSignalHeadSelected) {
        ...
        if (foundGroupingData->_mCalledOnExternalSensor->valid()) {
            sensors.insert(foundGroupingData->_mCalledOnExternalSensor->getBean());
        }
...
//  The route is the O.S. section that called us, along with the called on occupancy sensor:
        returnValue->_mLockedRoute = _mLockedRoutesManager->checkRouteAndAllocateIfAvailable(sensors, userIdentifier, "Rule #" + QString::number(ruleNumber), signalDirectionLever == CTCConstants::RIGHTTRAFFIC);
        if (returnValue->_mLockedRoute == nullptr) return returnValue;         // Not available, fake out
...
/*static*/ /*private*/ int CallOn::arrayFind(QStringList array, QString aString) {
    for (int index = 0; index < array.length(); index++) {
        if (aString == (array[index])) return index;
    }
    return -1;
}
...
/*private*/ QString CallOn::convertFromForeignLanguageColor(QString foreignLanguageColor) {
    ...
    if (foreignLanguageColor == (("SignalHeadStateYellow"))) color = "Yellow"; // NOI18N
    if (foreignLanguageColor == (("SignalHeadStateGreen"))) color = "Green";   // NOI18N
    if (foreignLanguageColor == (("SignalHeadStateFlashingRed"))) color = "Flashing Red";       // NOI18N
    if (foreignLanguageColor == (("SignalHeadStateFlashingYellow"))) color = "Flashing Yellow"; // NOI18N
    if (foreignLanguageColor == (("SignalHeadStateFlashingGreen"))) color = "Flashing Green";   // NOI18N
    if (foreignLanguageColor == (("SignalHeadStateLunar"))) color = "Lunar";                    // NOI18N
    if (foreignLanguageColor == (("SignalHeadStateFlashingLunar"))) color = "Flashing Lunar";   // NOI18N
    return color;
}
