...
///*public*/ class CTCMain {
  ...
  /*private*/ /*final*/ /*static*/ Logger* CTCMain::log = LoggerFactory::getLogger("CTCMain");

//  So that external python script can set locks on all of the lockable turnouts:
    /*public*/ void CTCMain::externalLockTurnout() {
        for (CodeButtonHandler* codeButtonHandler : _mCodeButtonHandlersArrayList) {
            codeButtonHandler->externalLockTurnout();
        }
    }

    /*public*/ CTCMain::CTCMain(QObject *parent) : QObject(parent) {
        CtcManager* mgr = (CtcManager*)InstanceManager::getDefault("CtcManager");
        _mCTCSerialData = mgr->getCTCSerialData();
        InstanceManager::store(_mCTCExceptionBuffer, "CTCExceptionBuffer");
    }
...
//  /*final*/ly, display errors to the user:
