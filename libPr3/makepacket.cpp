...
// /*public*/ class MakePacket {
    ...
    /*private*/ /*static*/ int MakePacket::preambleLength = 15;
     ...
     * @return bool true if preamble is a multiple of 5, otherwise fails and
     *         returns false
     */
    /*public*/ /*static*/ bool MakePacket::setPreambleLength(int preambleLen) {
     ...
     * @return QVector<int>        first byte is length - 0 length indicates failed to do
     */
    /*public*/ /*static*/ QVector<int> MakePacket::createStream(QByteArray packet) {
        int i = 0;
        int mask = 0x80;
        QVector<int> bitStream = QVector<int>(packet.length() * BITSTREAM_BITS_PER_BYTE
                + preambleLength + 1);
     ...
     */
    /*static*/ QVector<int> MakePacket::bitStreamToSerialBytes(QVector<int> inputBitStream) {
        int currentBufferIndex;
        int treeIndex = -1;
        QVector<int> serialStream =QVector<int> (inputBitStream.length());
        QVector<Node> tree =  QVector<Node>(150);

        for (currentBufferIndex = 0; currentBufferIndex < tree.length();
                currentBufferIndex++) {
            tree[currentBufferIndex] =  Node();
     ...
     */
    /*static*/ bool MakePacket::readNextChild(Node thisNode) {
    ...
    /*static*/ bool MakePacket::readFirstChild(QVector<int> bs,int offset, int validBits,
            Node thisNode) {
        bool b0 = false;
        bool b1 = false;
        bool b2 = false;
