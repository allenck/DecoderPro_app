...
/*public*/ InstallDecoderFileAction::InstallDecoderFileAction(QString s, QObject *parent) :
  InstallDecoderURLAction(s, (WindowInterface*)parent)
{
 //super(s);
 common();
}
...
QUrl* InstallDecoderFileAction::pickURL(QWidget* who)
{
 ...
 if (retVal == JFileChooser::APPROVE_OPTION)
 {
  ...
  {
   log->debug("located file " + file->toString() + " for XML processing");
  }
  try
  {
  ...
  {
   log->error("Unexpected exception in new URL: " + e->getMessage());
   return NULL;
  }
 }
 else
