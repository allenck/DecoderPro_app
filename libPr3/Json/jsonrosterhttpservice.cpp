...
{
 if (type  ==
      JsonRoster::ROSTER || type ==
      JsonRoster::ROSTER_ENTRY)
  return this->getRoster(locale, QJsonObject() /*this->mapper.createObjectNode()*/);
 else
 if(type ==  JsonRoster::ROSTER_GROUP ||
     type ==  JsonRoster::ROSTER_GROUPS)
 {
      return QJsonValue(this->getRosterGroups(locale));
 }else
...
{
 QString group = (!(data.value(JSON::GROUP).isNull()) ? (data.value(JSON::GROUP).toString()) : QString(""));
 if (Roster::ALLENTRIES == group || Roster::_AllEntries(locale) == group) {
        group = "";
}
 QString roadName = (!data.value(JSON::ROAD).isNull()) ? data.value(JSON::ROAD).toString() : NULL;
 QString roadNumber = (!data.value(JSON::NUMBER).isNull()) ? data.value(JSON::NUMBER).toString() : NULL;
 QString dccAddress = (!data.value(JSON::ADDRESS).isNull()) ? data.value(JSON::ADDRESS).toString() : NULL;
 QString mfg = (!data.value(JSON::MFG).isNull()) ? data.value(JSON::MFG).toString() : NULL;
 QString decoderModel = (!data.value(JSON::DECODER_MODEL).isNull()) ? data.value(JSON::DECODER_MODEL).toString() : NULL;
 QString decoderFamily = (!data.value(JSON::DECODER_FAMILY).isNull()) ? data.value(JSON::DECODER_FAMILY).toString() : NULL;
 QString id = (!data.value(JSON::NAME).isNull()) ? data.value(JSON::NAME).toString() : NULL;
...
/*public*/ QJsonObject JsonRosterHttpService::getRosterGroup(QLocale locale, QString name) throw (JsonException) {
    ...
    if (size != 0 || name == NULL || name.isEmpty() || name == (Roster::ALLENTRIES)) {
        ...
        throw JsonException(HttpServletResponse::SC_NOT_FOUND, tr("Error Not Found%1 %2").arg(JsonRoster::ROSTER_GROUP).arg(name));
    }
}
