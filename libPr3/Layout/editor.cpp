...
/*public*/ Editor::Editor(QObject* parent) : JmriJFrame(parent){
 commonInit();
}
void commonInit()
{
...
/*public*/ Editor::Editor(QString name, bool saveSize, bool savePosition, QObject* parent) : JmriJFrame(name,saveSize, savePosition, parent)
{
...
/*public*/ Editor::Editor(QString name, QObject* parent) : JmriJFrame(name, parent)
{
 commonInit();
    _debug = false;
   _loadFailed = false;
...
/*public*/ NamedIcon* Editor::loadFailed(QString msg, QString url) {
    ...
    QString goodUrl = _urlMap.get(url);
    if (goodUrl!=null) {
...
/*public*/ void Editor::setDefaultToolTip(ToolTip* dtt) {
    _defaultToolTip = dtt;
}
...
/*protected*/ Dimension* Editor::getTargetPanelSize() {
    return _targetPanel.getSize();
}

/*public*/ const JComponent* Editor::getTargetPanel() {
    return _targetPanel;
}
...
/*public*/ const JFrame Editor::getTargetFrame() {
    return _targetFrame;
}
...
/*public*/ void Editor::setBackgroundColor(Color col){
    TargetPane* tmp = (TargetPane*) _targetPanel;
...
/*protected*/ void Editor::setToolTip(ToolTip tt) {
    if (tt==null) {
        _tooltip = null;
 ...
 * @param state true for controlling.
 */
/*public*/ void setPanelMenu(bool state) {
 ...
 */
/*protected*/ void targetWindowClosing(bool save) {
...
/*protected*/ Editor changeView(QString className) {
    ...
    try {
        ...
        for (int i = 0; i<_contents.size(); i++) {
            ...
            p.setEditor(ed);
            ed.addToTarget(p);
            if (_debug) log.debug("changeView: "+p.getNameQString()+" addToTarget class= "
                                  +p.getClass().getName());
        }
...
*/
/*public*/ bool setShowCoordinatesMenu(Positionable p, JPopupMenu popup) {
    //if (showCoordinates()) {
...
*/
/*public*/ bool setShowAlignmentMenu(Positionable p, JPopupMenu popup) {
    if (showAlignPopup(p)) {
...
/*protected*/ void locoMarkerFromRoster() {
    const JmriJFrame locoRosterFrame = new JmriJFrame();
    ...
    const RosterEntrySelectorPanel rosterBox = new RosterEntrySelectorPanel();
...
/*public*/ void putItem(Positionable l) {
    ...
    addToTarget(l);
    if (!_contents.add(l)) {
        log.error("Unable to add "+l.getNameQString()+" to _contents");
    }
    if (_debug) log.debug("putItem "+l.getNameQString()+" to _contents. level= "+l.getDisplayLevel());
}
...
static const /*public*/ QString[] ICON_EDITORS = {"Sensor", "RightTurnout", "LeftTurnout",
...
/*protected*/ SensorIcon putSensor() {
    ...
    Hashtable <QString, NamedIcon> map = editor.getIconMap();
    Enumeration <QString> e = map.keys();
...
/*public*/ static class JFrameItem extends JmriJFrame {
    IconAdder _editor;
    JFrameItem (QString name, IconAdder editor) {
        super(name);
        _editor = editor;
        setName(name);
    }
...
/*protected*/ void makeSelectionGroup(MouseEvent event) {
    if (!event.isControlDown() || _selectionGroup==null) {
        _selectionGroup = new QVector <Positionable>();
    }
...
/*protected*/ bool setTextAttributes(Positionable p, JPopupMenu popup) {
