        ...
        : AbstractNamedBean(sName.toUpper(), uName, parent)
{
 ...
 sensorDebounceGlobalCheck->setText(tr("OccupancySensorUseGlobal"));
...
// blockEditCancel;
 ...
 actedUponUpdates = new QVector<int>();
    ...
    blockTrackColor = QColor::black;
    blockOccupiedColor = QColor::black;
    blockExtraColor = QColor::black;
...
/*protected*/ void  LayoutBlock::initializeLayoutBlock() {
    ...
    if (block!=NULL) {
        ...
        block->addPropertyChangeListener(mBlockListener = new PropertyChangeListener());// {
 ...
 * Returns Memory name
*/
/*public*/ QString  LayoutBlock::getMemoryName() {return (memoryName);}

/**
 * Returns Memory
*/
/*public*/ Memory*  LayoutBlock::getMemory() {return (memory);}
...
/*public*/ void  LayoutBlock::setOccupancySensorName(QString name)
{
 ...
 if (sensor!=NULL)
 {
  occupancyNamedSensor =InstanceManager::::getDefault("NamedBeanHandleManager").getNamedBeanHandle(name, sensor);
        if (block!=NULL)
            block.setNamedSensor(occupancyNamedSensor);
    }
}
...
/*public*/ void  LayoutBlock::setOccupiedSense(int sense) {occupiedSense = sense;}
...
/*public*/ void LayoutBlock::setState(int i) { log.error("this state does nothing " + getDisplayName());}
...
//JCheckBox sensorDebounceGlobalCheck = new JCheckBox(tr("OccupancySensorUseGlobal"));
...
///*JLabel*/ QLabel blockUseLabel= new /*JLabel*/ QLabel( tr("UseCount"));
...
/*protected*/ void  LayoutBlock::editLayoutBlock(/*Component*/ QWidget* callingPane)
{
    ...
    if (editLayoutBlockFrame == NULL) {
        editLayoutBlockFrame = new JmriJFrame( tr("EditBlock"), false, false);
        ...
        /*JLabel*/ QLabel sensorLabel = new /*JLabel*/ QLabel( tr("OccupancySensor")+":");
        panel3.add(sensorLabel);
        panel3.add(sensorNameField);
        sensorNameField.setToolTipText( tr("OccupancySensorToolTip") );
        contentPane.add(panel3);

        // set up occupied sense (changeable)
        JPanel panel4 = new JPanel();
        panel4.setLayout(new FlowLayout());
        /*JLabel*/ QLabel sensorSenseLabel = new /*JLabel*/ QLabel( tr("OccupiedSense")+":");
        panel4.add(sensorSenseLabel);
        senseBox.removeAllItems();
        senseBox.addItem( tr("SensorActive") );
        senseActiveIndex = 0;
        senseBox.addItem( tr("SensorInactive") );
        senseInactiveIndex = 1;
        panel4.add(senseBox);
        senseBox.setToolTipText( tr("OccupiedSenseHint") );
        contentPane.add(panel4);

        contentPane.add(sensorDebounceGlobalCheck);
        sensorDebounceGlobalCheck.setToolTipText(tr("OccupancySensorUseGlobal"));
        JPanel panel4a = new JPanel();
        panel4a.add(new /*JLabel*/ QLabel(tr("OccupancySensorDebounceInActive")));
        sensorDebounceInactiveField.setToolTipText(tr("OccupancySensorDebounceInActiveToolTip"));
        panel4a.add(sensorDebounceInactiveField);
        contentPane.add(panel4a);
        JPanel panel4b = new JPanel();
        panel4b.add(new /*JLabel*/ QLabel(tr("OccupancySensorDebounceActive")));
        panel4b.add(sensorDebounceActiveField);
        sensorDebounceActiveField.setToolTipText(tr("OccupancySensorDebounceActiveToolTip"));
        ...
        memoryNameField.setToolTipText( tr("MemoryVariableTip") );
        ...
        if(InstanceManager::layoutBlockManagerInstance().isAdvancedRoutingEnabled()){
            ...
            neighbourDir = new QVector<JComboBox>(getNumberOfNeighbours());
            for(int i = 0; i<getNumberOfNeighbours(); i++){
                JPanel panel = new JPanel();
                panel.setLayout(new FlowLayout());
                panel.add(new /*JLabel*/ QLabel("Attached Block: " + getNeighbourAtIndex(i).getDisplayName()));
        ...
        // Done
        panel5.add(blockEditDone = new JButton(tr("Done")));
        blockEditDone.addActionListener(new ActionListener() {
            /*public*/ void actionPerformed(ActionEvent e) {
                blockEditDonePressed(e);
            }
        });
        blockEditDone.setToolTipText( tr("DoneHint") );
        // Cancel
        panel5.add(blockEditCancel = new JButton(tr("Cancel")));
        blockEditCancel.addActionListener(new ActionListener() {
            /*public*/ void actionPerformed(ActionEvent e) {
                blockEditCancelPressed(e);
            }
        });
        blockEditCancel.setToolTipText( tr("CancelHint") );
        contentPane.add(panel5);


    }
    // Set up for Edit
    blockUseLabel.setText(tr("UseCount")+": "+useCount );
    ...
    if(getOccupancySensor()!=NULL){
        ...
        sensorDebounceInactiveField.setText(QString.valueOf(getOccupancySensor().getSensorDebounceGoingInActiveTimer()));
        sensorDebounceActiveField.setText(QString.valueOf(getOccupancySensor().getSensorDebounceGoingActiveTimer()));
        sensorDebounceGlobalCheck.setSelected(getOccupancySensor().useDefaultTimerSettings());
    }
    sensorDebounceGlobalCheck.addActionListener(new ActionListener() {
        /*public*/ void actionPerformed(ActionEvent e){
            if(sensorDebounceGlobalCheck.isSelected()){
                ...
                sensorDebounceActiveField.setText(QString.valueOf(jmri.InstanceManager::sensorManagerInstance().getDefaultSensorDebounceGoingActive()));
                sensorDebounceInactiveField.setText(QString.valueOf(jmri.InstanceManager::sensorManagerInstance().getDefaultSensorDebounceGoingInActive()));
            } else {
    ...
    metricField.setText(Integer.toQString(metric));
...
QString[] working = {"Bi-Directional", "Recieve Only", "Send Only"};
...
void blockEditDonePressed(ActionEvent a) {
    ...
    if ( !(getOccupancySensorName()).equals(sensorNameField.getText().trim()) ) {
        // sensor has changed
        QString newName = sensorNameField.getText().trim();
    ...
    if(getOccupancySensor()!=NULL){
        ...
        if(getOccupancySensor().getReporter()!=NULL && block!=NULL){
            QString msg = java.text.MessageFormat.format(rb
                    .getQString("BlockAssignReporter"), new Object[] { getOccupancySensor().getDisplayName(), getOccupancySensor().getReporter().getDisplayName() });
    ...
    if ( !memoryName.equals(memoryNameField.getText().trim()) ) {
        // memory has changed
        QString newName = memoryNameField.getText().trim();
 ...
 */
QString[] colorText = {"Black","DarkGray","Gray",
...
/*private*/ void initializeColorCombo(JComboBox colorCombo) {
    ...
    for (int i = 0;i<numColors;i++) {
        colorCombo.addItem( tr(colorText[i]) );
    }
}
 ...
 */
/*public*/ static QString colorToQString(Color color) {
...
/*public*/ static Color stringToColor(QString string) {
...
/*public*/ bool useDefaultMetric() { return defaultMetric; }
...
void notifyNeighboursOfRemoval(QVector<Routes> routesToRemove, Block notifyingblk){
    QString msgPrefix = "From " + this.getDisplayName() + " notify block " + notifyingblk.getDisplayName() + " ";
    if(enableDeleteRouteLogging)
...
void addThroughPath(Block srcBlock, Block dstBlock, LayoutEditor panel){
    ...
    QVector<Integer> stodSet = new QVector<Integer>();
    ...
    QVector<Integer> tmpdtosSet = new QVector<Integer>();
    ...
    if ((stod.size()==tmpdtos.size()) && (stodSet.size()==tmpdtosSet.size())){
        ...
        QVector<Integer> dtosSet = new QVector<Integer>();
...
/*private*/ void addThroughPathPostChecks(Block srcBlock, Block dstBlock, QVector<LayoutTurnout> stod, QVector<Integer> stodSet ){
        java.util.List<jmri.Path> paths = block.getPaths();
...
//was /*public*/
bool updatePacketActedUpon(Integer packetID){
...
/*public*/ void printRoutes(){
    ...
    for(int i = 0; i<routes.size(); i++){
        ...
        log.info((r.getDestBlock()).getDisplayName() + ", " + nexthop + ", " + r.getHopCount() + ", " + Path.decodeDirection(r.getDirection()) + ", " + r.getState() + ", " + r.getMetric() + activeQString);
    }
}
...
//@Override
/*protected*/ void firePropertyChange(QString p, Object old, Object n) { pcs.firePropertyChange(p,old,n);}

/*public*/ void propertyChange(java.beans.PropertyChangeEvent e) {
    if(e.getSource() instanceof LayoutBlock){
        LayoutBlock srcEvent = (LayoutBlock) e.getSource();
        if (e.getPropertyName().toQString().equals("NewRoute")){
            LayoutBlock lbkblock = (LayoutBlock) e.getNewValue();
            if(enableUpdateRouteLogging) log.info("==Event type " + e.getPropertyName().toQString() + " New " + lbkblock.getDisplayName());
        } else if (e.getPropertyName().toQString().equals("through-path-added")) {
            if(enableUpdateRouteLogging) log.info("neighbour has new through path");
        } else if (e.getPropertyName().toQString().equals("through-path-removed")) {
            if(enableUpdateRouteLogging) log.info("neighbour has through removed");
        } else if (e.getPropertyName().toQString().equals("routing")){
            if(enableUpdateRouteLogging)
...
*/
/*public*/ QString getNeighbourPacketFlowAsQString(int i){
...
Adjacencies::Adjacencies(Block* block, int dir, int packetFlow)
{
...
*/
/*public*/ QString getRouteStateAsQString(int i){
...
/*public*/ Routes::Routes(Block dstBlock, Block nxtBlock, int hop, int dir, int met, float len)
{
    miscflags =0x00;
...
ThroughPaths::ThroughPaths(Block* srcBlock, Path* srcPath, Block* destBlock, Path* dstPath)
{
...
QVector<Block*> ThroughPaths::getThroughPathSourceByDestination(Block* dest){
...
QVector<Block*> ThroughPaths::getThroughPathDestinationBySource(Block* source){
    QVector<Block*> a = new QVector<Block*>();
    for(int i = 0; i<throughPaths.size(); i++){
...
bool ThroughPaths::checkIsRouteOnValidThroughPath(Routes* r){
    for(int i = 0; i<throughPaths.size(); i++){
...
/*public*/ void ThroughPaths::refreshValidRoutes(){
    for(int i= 0; i<throughPaths.size(); i++){
        ThroughPaths t = throughPaths.get(i);
...
void ThroughPaths::updateActiveThroughPaths(ThroughPaths* tp, bool active){
    if(enableUpdateRouteLogging)
...
void ThroughPaths::setRoutesValid(Block* nxtHopActive, bool state){
