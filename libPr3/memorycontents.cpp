...
//MemoryContents::MemoryContents(QObject *parent) :
//  QObject(parent)
//{
//}
/**
...
// /*public*/ class MemoryContents {
     ...
     */
    /*private static final*/ int MemoryContents::DEFAULT_MEM_VALUE = -1;
    /*private static final*/ int MemoryContents::PAGESIZE = 0x10000;
    /*private static final*/ int MemoryContents::PAGES = 256;

    /*private static final*/ QString MemoryContents::STRING_DATA_RECTYP = StringUtil::twoHexFromInt(RECTYP_DATA_RECORD);
    /*private static final*/ int MemoryContents::CHARS_IN_RECORD_MARK = 1;
    /*private static final*/ int MemoryContents::CHARS_IN_RECORD_LENGTH = 2;
    /*private static final*/ int MemoryContents::CHARS_IN_RECORD_TYPE = 2;
    /*private static final*/ int MemoryContents::CHARS_IN_EACH_DATA_BYTE = 2;
    /*private static final*/ int MemoryContents::CHARS_IN_CHECKSUM = 2;
    /*private static final*/ int MemoryContents::CHARS_IN_24_BIT_ADDRESS = 6;
    /*private static final*/ int MemoryContents::CHARS_IN_16_BIT_ADDRESS = 4;

    /*private static final*/ QChar MemoryContents::LEADING_CHAR_COMMENT = '#'; // NOI18N
    /*private static final*/ QChar MemoryContents::LEADING_CHAR_KEY_VALUE = '!'; // NOI18N
    /*private static final*/ QChar MemoryContents::LEADING_CHAR_RECORD_MARK = ':'; // NOI18N
 ...
 *      :00000001FF
 */
/*public*/ MemoryContents::MemoryContents(QObject *parent) :
QObject(parent)
{
 log = new Logger("MemoryContents");
 keyValComments =QStringList() << "";
 loadOffsetFieldType = UNDEFINED;
 //pageArray = int[/*PAGES*/256][];
 pageArray = new QVector< QVector<int>* >(256);
 currentPage = -1;
 hasData = false;
 curExtLinAddr = 0;
 curExtSegAddr = 0;
...
/*private*/ bool MemoryContents::isPageInitialized(int page)
{
 //return (pageArray[page] != NULL);
 return pageArray->at(page) != NULL;
}
 ...
 * @param page
 */
/*private*/ void MemoryContents::initPage(int page)
{
 //if (pageArray[page] != NULL)
 if(pageArray->at(page))
 {
 ...
 //int largeArray[PAGESIZE];// = new int[PAGESIZE];
 QVector<int>* largeArray = new QVector<int>(PAGESIZE, DEFAULT_MEM_VALUE);
...
// }
 ...
 pageArray->replace(page, largeArray);
#endif
}
 ...
 */
/*public*/ void MemoryContents::readHex(QString filename) //throws FileNotFoundException,
 ...
 *                                             via newBufferedReader
 */
/*public*/ void MemoryContents::readHex(File* file) //throws FileNotFoundException,
//        MemoryFileRecordLengthException, MemoryFileChecksumException,
//        MemoryFileUnknownRecordType, MemoryFileRecordContentException,
//        MemoryFileNoDataRecordsException, MemoryFileNoEOFRecordException,
//        MemoryFileRecordFoundAfterEOFRecord, MemoryFileAddressingRangeException,
//        IOException
{
...
#endif
 QFile inFile(file->getPath());
 if(!inFile.open(QIODevice::ReadOnly | QIODevice::Text))
 {
  QString err = tr("File '%1' not found").arg(file->getPath());
  emit memoryContentsException(FileNotFoundException, err);
  log->error(err);
  return;
 }
 ...
 currentPage = 0;
 loadOffsetFieldType = UNDEFINED;
 bool foundDataRecords = false;
 bool foundEOFRecord = false;
...
//    try {
 ...
 while ((line = fileStream.readLine()) != NULL)
 {
  ...
  if (line.at(0) == LEADING_CHAR_COMMENT)
  {
   // human comment.  Ignore it.
  }
  else if (line.at(0) == LEADING_CHAR_KEY_VALUE)
  {
   // machine comment; store it to allow for key/value extraction
   keyValComments.append(line);
  }
  else if (line.at(0) == LEADING_CHAR_RECORD_MARK)
  {
   ...
   if (loadOffsetFieldType == UNDEFINED)
   {
            ...
            && (isLoadOffsetType24Bits()))
    {
     ...
     QString message = tr("Could not infer addressing type from") // NOI18N
             + " line " + QString::number(lineNum) + "."; // NOI18N
     log->error(message);
     //throw new MemoryFileRecordContentException(message);
     emit memoryContentsException(MemoryFileRecordContentException, message);
     return;
    }
   }
   ...
   if (indexOfLastAddressCharacter < 0)
   {
    ...
    QString message = "Fell thru with unknown loadOffsetFieldType value " // NOI18N
            + QString::number(loadOffsetFieldType) + " for line " + QString::number(lineNum) + "."; // NOI18N
    log->error(message);
    //throw new MemoryFileAddressingRangeException(message);
    emit memoryContentsException(MemoryFileAddressingRangeException, message);
    return;
   }

   // extract the RECTYP.
   int recordType = line.mid(indexOfLastAddressCharacter + 1,
           /*indexOfLastAddressCharacter + 3*/2).toInt(0, 16);
   if (log->isDebugEnabled())
   {
    log->debug("RECTYP = 0x" // NOI18N
            + QString::number(recordType,16));
   }
           ...
           + (count * CHARS_IN_EACH_DATA_BYTE) + CHARS_IN_CHECKSUM)
   {
    ...
    QString message
            = tr("Data record line length is incorrect for ") // NOI18N
            + "inferred addressing type and for data " // NOI18N
            + "count field in line " + QString::number(lineNum);// NOI18N
    log->error(message);
    //throw new MemoryFileRecordLengthException(message);
    emit memoryContentsException(MemoryFileRecordLengthException, message);
    return;
   }
   ...
   int computedChecksum = calculate8BitChecksum(line.mid(CHARS_IN_RECORD_MARK));
   if (computedChecksum != 0x00)
   {
    ...
    int expectedChecksum = line.mid(line.length() - 2).toInt(0, 16);
    QString message = "Record checksum error in line " // NOI18N
            + QString::number(lineNum)
            + " - computed checksum = 0x" // NOI18N
            + QString::number(computedChecksum,16)
            + ", expected checksum = 0x" // NOI18N
            + QString::number(expectedChecksum,16)
            + "."; // NOI18N
    log->error(message);
    //throw new MemoryFileChecksumException(message);
    emit memoryContentsException(MemoryFileChecksumException, message);
    return;
   }
   ...
   if (recordType == RECTYP_DATA_RECORD)
   {
    ...
    if (foundEOFRecord)
    {
     ...
     QString message = "Found a Data record in line " // NOI18N
             + QString::number(lineNum) + " after the EOF record"; // NOI18N
     log->error(message);
//     throw new MemoryFileRecordFoundAfterEOFRecord(message);
     emit memoryContentsException(MemoryFileRecordFoundAfterEOFRecord, message);
     return;
    }
    ...
    if (addressAndCountIsOk(effectiveAddress, count) == false)
    {
     ...
     // So refuse the file.
     QString message = tr("Data crosses boundary which could lead to ") // NOI18N
...
//     throw new MemoryFileAddressingRangeException(message);
    ...
    if (!isPageInitialized(effectivePage))
    {
     ...
     log->debug(tr("effective address 0x%1 is causing change to segment 0x%2").arg( // NOI18N
                                                                                    QString::number(effectiveAddress,16)).arg(
             QString::number(effectivePage,16)));
    }
    int effectiveOffset = effectiveAddress % PAGESIZE;

    log->debug(tr("Effective address 0x%1, effective page 0x%2, effective offset 0x%3").arg(
                QString::number(effectiveAddress,16)).arg(
                QString::number(effectivePage, 16)).arg(
            QString::number(effectiveOffset,16)));
    for (int i = 0; i < count; ++i)
    {
     int startIndex = indexOfLastAddressCharacter + 3 + (i * 2);
     // parse as hex into integer, then convert to byte
     ival = line.mid(startIndex, /*startIndex +*/ 2).toInt(0, 16);
     //pageArray[effectivePage][effectiveOffset++] = ival;
     QVector<int>* largeArray = pageArray->at(effectivePage);
     largeArray->replace(effectiveOffset++, ival);
     hasData = true;
    }
   ...
   else if (recordType == RECTYP_EXTENDED_SEGMENT_ADDRESS_RECORD)
   {
    ...
    if (datacount != 2)
    {
     QString message = tr("Extended Segment Address record ") // NOI18N
...
//     throw new MemoryFileRecordContentException(message);
    ...
    int newPage = 16 * line.mid(startpoint,
            (startpoint + 2 * datacount)-startpoint).toInt(0, 16);

    // check for an allowed segment value
    if (newPage != 0)
    {
     QString message = tr("Unsupported Extended Segment Address ") // NOI18N
             + "Record data value 0x" // NOI18N
             + QString::number(newPage, 16)
             + " in line " + QString::number(lineNum); // NOI18N
     log->error(message);
     //throw new MemoryFileAddressingRangeException(message);
     emit memoryContentsException(MemoryFileAddressingRangeException, message);
     return;
...
//     throw new MemoryFileRecordFoundAfterEOFRecord(message);
    ...
    {
     QString message = tr("Extended Linear Address record has ") // NOI18N
...
//     throw new MemoryFileRecordContentException(message);
    ...
    if (datacount != 2)
    {
     QString message = tr("Expect data payload length of 2, ") // NOI18N
             + tr("found RECLEN value of ")  // NOI18N
             + QString::number(extractRecLen(line))
             + " in line " + QString::number(lineNum); // NOI18N
     log->error(message);
//     throw new MemoryFileRecordContentException(message);
     emit memoryContentsException(MemoryFileRecordContentException, message);
    ...
    int startpoint = indexOfLastAddressCharacter + 3;
    int tempPage = line.mid(startpoint,
            (startpoint + 2 * datacount)-startpoint).toInt(0, 16);

    if ((tempPage != 0) && (isLoadOffsetType24Bits()))
    {
     // disallow non-zero extended linear address if 24-bit addressing
     QString message = tr("Extended Linear Address record with non-zero") // NOI18N
             + tr("data field in line ") // NOI18N
             + QString::number(lineNum)
...
//     throw new MemoryFileRecordContentException(message);
    ...
    else if (tempPage < PAGES)
    {
     ...
     if (log->isDebugEnabled())
     {
      log->debug("New page 0x" + QString::number(currentPage,16)); // NOI18N
     } // NOI18N
    }
    else
    {
     QString message = "Page number 0x" // NOI18N
             + QString::number(tempPage, 16)
...
//     throw new MemoryFileAddressingRangeException(message);
   ...
   else if (recordType == RECTYP_EOF_RECORD)
   {
    ...
    {
     QString message = "Illegal EOF record form in line "
             + QString::number(lineNum);
     log->error(message);
//     throw new MemoryFileRecordContentException(message);
     emit memoryContentsException(MemoryFileRecordContentException, message);
   ...
   {
    QString message = "Unknown RECTYP 0x" // NOI18N
            + QString::number(recordType,16)
            + " was found in line " // NOI18N
            + QString::number(lineNum) + ".  Aborting file read."; // NOI18N
    log->error(message);
//    throw new MemoryFileUnknownRecordType(message);
    emit memoryContentsException(MemoryFileUnknownRecordType, message);
    return;
   }
  ...
  {
   QString message = tr("Unknown line type in line ") + QString::number(lineNum) + "."; // NOI18N
   log->error(message);
//   throw new MemoryFileUnknownRecordType(message);
   emit memoryContentsException(MemoryFileUnknownRecordType, message);
   return;
...
//}
 if (!foundDataRecords)
 {
  QString message = "No Data Records found in file - aborting."; // NOI18N
  log->error(message);
  //throw new MemoryFileNoDataRecordsException(message);
  emit memoryContentsException(MemoryFileNoDataRecordsException, message);
  return;
 }
 else if (foundDataRecords && !foundEOFRecord)
 {
  QString message = "No EOF Record found in file - aborting."; // NOI18N
  log->error(message);
 }
}
 ...
 */
/*public*/ void MemoryContents::writeHex(Writer* w) //throws IOException, MemoryFileAddressingFormatException
{
    writeHex(w, 16);
}
 ...
 * @throws MemoryFileAddressingFormatException
 */
/*public*/ void MemoryContents::writeHex(Writer* writer, bool writeKeyVals, int blockSize)
 ...
 * @throws MemoryFileAddressingFormatException
 */
/*private*/ void MemoryContents::writeHex(Writer* writer, int blockSize)
//        throws IOException, MemoryFileAddressingFormatException
{
#if 0
    ...
    if ((!isLoadOffsetType16Bits())
            && (!isLoadOffsetType24Bits())) {
        QString message = "Invalid loadOffsetFieldType at writeHex invocation"; // NOI18N
        log->error(message);
 ...
 */
/*public*/ int MemoryContents::nextContent(int location)
{
 ...
 */
/*public*/ void MemoryContents::setLocation(int location, int value) {
    currentPage = location / PAGESIZE;

 //pageArray[currentPage][location % PAGESIZE] = value;
 QVector<int>* largeArray = pageArray->at(currentPage);
 largeArray->replace(location % PAGESIZE, value);
 hasData = true;
}
 ...
 *         memory
 */
/*public*/ bool MemoryContents::locationInUse(int location)
{
 currentPage = location / PAGESIZE;
 //if (pageArray[currentPage] == NULL)
 if(pageArray->at(currentPage) == NULL)
 ...
 */
/*public*/ int MemoryContents::getLocation(int location)
{
 currentPage = location / PAGESIZE;
 //if (pageArray[currentPage] == NULL)
 if(pageArray->at(currentPage) == NULL)
 ...
 */
/*public*/ bool MemoryContents::isEmpty() {
    return !hasData;
}
 ...
 */
/*private*/ LoadOffsetFieldType MemoryContents::inferRecordAddressType(QString recordString)
 {
    if (recordString.at(0) != LEADING_CHAR_RECORD_MARK) {
        log->error("Cannot infer record addressing type because line " // NOI18N
                + QString::number(lineNum)
                + " is not a record."); // NOI18N
        return ADDRESSFIELDSIZEUNKNOWN;
    }
    QString r = recordString.mid(CHARS_IN_RECORD_MARK);  // create a string without the leading ':'
    int len = r.length();
    if (((len + 1) / 2) != (len / 2)) {
    ...
    int datalen = r.mid(0, 2).toInt(0, 16);
    int checksumInRecord = r.mid(len - 2, /*len*/2).toInt(0, 16);

    // Compute the checksum of the record
//    int calculatedChecksum = calculate8BitChecksum(recordString.mid(CHARS_IN_RECORD_MARK,
//            recordString.length() - CHARS_IN_CHECKSUM));
    int calculatedChecksum = calculate8BitChecksum(recordString.mid(CHARS_IN_RECORD_MARK,
            recordString.length() - CHARS_IN_CHECKSUM - CHARS_IN_RECORD_MARK));
    ...
    if (calculatedChecksum != checksumInRecord) {
        log->error("Cannot infer record addressing type because line " // NOI18N
                ...
                + QString::number(calculatedChecksum,16)
                + ", found CHKSUM = 0x" // NOI18N
                + QString::number(checksumInRecord,16)
                + ")"); // NOI18N
        return ADDRESSFIELDSIZEUNKNOWN;
    }
    ...
    if (computedLenIf16Bit == len) {
        ...
        if (isSupportedRecordType(r.mid(6, /*8*/2).toInt(0, 16))) {
            return ADDRESSFIELDSIZE16BITS;
        } else {
    ...
    if (computedLenIf24Bit == len) {
        ...
        if (isSupportedRecordType(r.mid(8, /*10*/2).toInt(0, 16))) {
            return ADDRESSFIELDSIZE24BITS;
        } else {
            log->error("Cannot infer record addressing type in line " // NOI18N
                    + QString::number(lineNum)
 ...
 */
/*private*/ int MemoryContents::calculate8BitChecksum(QString infoString)
{
 ...
 {
  //calculatedChecksum += infoString.mid(i, i + 2).toInt(0, 16);
  calculatedChecksum += infoString.mid(i, 2).toInt(0, 16);
 }
 ...
 *         false.
 */
/*private*/ bool MemoryContents::addressAndCountIsOk(int addr, int count) {
    int beginPage = addr / PAGESIZE;
    int endPage = ((addr + count - 1) / PAGESIZE);
    log->debug("Effective Record Addr = 0x" + QString::number(addr,16) // NOI18N
            + " count = " + QString::number(count) // NOI18N
            + " BeginPage = " + QString::number(beginPage) // NOI18N
            + " endpage = " + QString::number(endPage)); // NOI18N
 ...
 */
/*public*/ QString MemoryContents::extractValueOfKey(QString keyName) {
    for (int i = 0; i < keyValComments.size(); i++) {
        QString t = keyValComments.at(i);
        QString targetedKey = "! " + keyName + ": "; // NOI18N
        if (t.startsWith(targetedKey)) {
            int f = t.indexOf(": "); // NOI18N
            QString value = t.mid(f + 2, t.length());
            if (log->isDebugEnabled()) {
 ...
 */
/*private*/ int MemoryContents::findKeyCommentIndex(QString keyName) {
    for (int i = 0; i < keyValComments.size(); i++) {
        QString t = keyValComments.at(i);
        QString targetedKey = "! " + keyName + ": "; // NOI18N
        if (t.startsWith(targetedKey)) {
 ...
 */
/*public*/ void MemoryContents::setAddressFormat(LoadOffsetFieldType addressingType) {
 ...
 */
/*public*/ LoadOffsetFieldType MemoryContents::getCurrentAddressFormat() {
 ...
 * @throws IOException
 */
/*public*/ void MemoryContents::writeComments(Writer* writer) //throws IOException
 {
#if 0
    foreach (QString s, keyValComments) {
        writer->write(s);
    }
#endif
}

/*private*/ bool MemoryContents::isLoadOffsetType24Bits() {
    return loadOffsetFieldType == ADDRESSFIELDSIZE24BITS;
}

/*private*/ bool MemoryContents::isLoadOffsetType16Bits() {
    return loadOffsetFieldType == ADDRESSFIELDSIZE16BITS;
}

/*private*/ bool MemoryContents::isSupportedRecordType(int recordType) {
    switch (recordType) {
...
/*private*/ int MemoryContents::extractRecLen(QString line) {
    return line.mid(CHARS_IN_RECORD_MARK,
            /*CHARS_IN_RECORD_MARK +*/ CHARS_IN_RECORD_LENGTH).toInt(0, 16);
}

/*private*/ int MemoryContents::charsInAddress() {
    if (isLoadOffsetType24Bits()) {
...
/*private*/ int MemoryContents::extractLoadOffset(QString line) {
 return line.mid(CHARS_IN_RECORD_MARK + CHARS_IN_RECORD_LENGTH,
                    /*CHARS_IN_RECORD_MARK + CHARS_IN_RECORD_LENGTH +*/ charsInAddress()).toInt(0, 16);
}
#if 0
/**
