...
void AudioInputStream::common()
{
 pushBackBuffer = NULL;
 pushBackLen = 0;
 markPushBackBuffer = NULL;
 markPushBackLen = 0;
 log = new Logger("AudioInputStream");
}
 ...
 */
/*public*/ AudioInputStream::AudioInputStream(QDataStream* stream, QAudioFormat* format, long length, QObject *parent) :
  QObject(parent)
{
 //super();
 common();
 ...
 //this->frameSize = /*format->getFrameSize()*/format->bytesPerFrame(); // number of bytes in each frame
 this->frameSize = format->sampleSize()/8;
...
/*public*/ AudioInputStream(TargetDataLine line) {
    ...
    frameLength = /*AudioSystem::NOT_SPECIFIED*/-1;
    frameSize = format.getFrameSize();

    if( frameSize == /*AudioSystem::NOT_SPECIFIED*/-1 || frameSize <= 0) {
        frameSize = 1;
    }
    this->stream = tstream;
    framePos = 0;
 ...
 */
/*public*/ QAudioFormat* AudioInputStream::getFormat() {
    return format;
}
 ...
 */
/*public*/ long AudioInputStream::getFrameLength() {
    return frameLength;
}
 ...
 * <p>
 */
/*public*/ int AudioInputStream::read() /*throws IOException*/
{
 if( frameSize != 1 )
 {
  //throw new IOException("cannot read a single byte if frame size > 1");
  log->error("cannot read a single byte if frame size > 1");
 }

 char data[1];
 int temp = stream->readRawData(data,1);
 if (temp <= 0)
 {
 ...
 * @see #available
 */
/*public*/ int AudioInputStream::read(QByteArray* b) /*throws IOException */{
 ...
 * @see #available
 */
/*public*/ int AudioInputStream::read(QByteArray* b, int off, int len) /*throws IOException*/
{
 ...
 if( frameLength != /*AudioSystem::NOT_SPECIFIED*/-1 )
 {
...
//                    b, off, pushBackLen);
  //pushBackBuffer->replace(0, pushBackLen, b->constData()+off);
  b->replace(off, pushBackLen,pushBackBuffer->constData()+0);
  thisOff += pushBackLen;
  len -= pushBackLen;
 ...
 int thisBytesRead = stream->readRawData(buf, len);
 b->replace(thisOff,thisBytesRead,buf, len);
 delete buf;
 ...
 if (bytesRead > 0)
 {
  ...
  if (pushBackLen > 0)
  {
   ...
   {
    pushBackBuffer = new QByteArray(frameSize, ' ');
   }
//   System.arraycopy(b, off + bytesRead - pushBackLen,
//                    pushBackBuffer, 0, pushBackLen);
   //b->replace(off + bytesRead - pushBackLen, pushBackLen, pushBackBuffer->constData() + 0);
   pushBackBuffer = &pushBackBuffer->replace(0, pushBackLen, b->constData() + off + bytesRead - pushBackLen);
   bytesRead -= pushBackLen;
  }
 ...
 * @see #available
 */
/*public*/ long AudioInputStream::skip(long n) /*throws IOException */
{
 ...
 long temp = stream->skipRawData(n);
 ...
 if( temp%frameSize != 0 )
 {
  ...
  log->error("Could not skip an integer number of frames.");
 }
 if( temp >= 0 )
 {
 ...
 * @see #skip
 */
/*public*/ int AudioInputStream::available() /*throws IOException*/
{
// int temp = stream->available();
 int temp = stream->device()->bytesAvailable();
 ...
 * @throws IOException if an input or output error occurs
 */
/*public*/ void AudioInputStream::close() /*throws IOException*/ {
    //stream->close();
 delete stream;
}
...
/*public*/ void AudioInputStream::mark(int readlimit) {
 ...
 * @see #markSupported
 */
/*public*/ void AudioInputStream::reset() /*throws IOException*/
{
 stream->reset();
 framePos = markpos;
 ...
 if (pushBackLen > 0)
 {
  ...
  markPushBackBuffer->replace(0, pushBackLen, pushBackBuffer->constBegin()+0);
 }
}
#endif
 ...
 * @see #reset
 */
/*public*/ bool AudioInputStream::markSupported() {
