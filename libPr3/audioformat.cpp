...
AudioFormat::AudioFormat(QObject *parent)
 :  QObject(parent)
{
 this->encoding = new Encoding("");
 this->sampleRate = 0;
 this->sampleSizeInBits = 0;
 this->channels = AudioSystem::NOT_SPECIFIED;
 this->frameSize = 0;
 this->frameRate = 0;
 ...
 *                                                  means little-endian)
 */
/*public*/ AudioFormat::AudioFormat(Encoding* encoding, float sampleRate, int sampleSizeInBits, int channels, int frameSize, float frameRate, bool bigEndian, QObject *parent)
 :  QObject(parent)
{
...
/*public*/ AudioFormat::AudioFormat(Encoding* encoding, float sampleRate,
                   ...
                   bool bigEndian, QMap<QString, QVariant>* properties, QObject *parent)
 : QObject(parent)
{
...
//         frameSize, frameRate, bigEndian);
 ...
 this->frameRate = frameRate;
 this->bigEndian = bigEndian;
 this->_properties = new QMap<QString, QVariant>(*_properties);
}
 ...
 *                                                  means little-endian)
 */
/*public*/ AudioFormat::AudioFormat(float sampleRate, int sampleSizeInBits,
                   int channels, bool _signed, bool bigEndian, QObject *parent) :
  QObject(parent)
{
...
//         sampleRate,
//         bigEndian);
 this->encoding = (_signed == true ? Encoding::PCM_SIGNED : Encoding::PCM_UNSIGNED);
 this->sampleRate = sampleRate;
 this->sampleSizeInBits = sampleSizeInBits;
 this->channels = (channels == AudioSystem::NOT_SPECIFIED || sampleSizeInBits == AudioSystem::NOT_SPECIFIED)?
             AudioSystem::NOT_SPECIFIED:
             ((sampleSizeInBits + 7) / 8) * channels;
 //this->frameSize = frameSize;
 ...
 * @see Encoding#ALAW
 */
/*public*/ Encoding* AudioFormat::getEncoding() {
 ...
 */
/*public*/ float AudioFormat::getSampleRate() {
 ...
 * @see AudioSystem#NOT_SPECIFIED
 */
/*public*/ int AudioFormat::getSampleSizeInBits() {
 ...
 * @see AudioSystem#NOT_SPECIFIED
 */
/*public*/ int AudioFormat::getChannels() {
 ...
 * @see AudioSystem#NOT_SPECIFIED
 */
/*public*/ int AudioFormat::getFrameSize() {
 ...
 * @see AudioSystem#NOT_SPECIFIED
 */
/*public*/ float AudioFormat::getFrameRate()
{
 return frameRate;
}
 ...
 */
/*public*/ bool AudioFormat::isBigEndian()
{
 return bigEndian;
}
 ...
 * @since 1.5
 */
/*public*/ QMap<QString,QVariant>* AudioFormat::properties() {
    QMap<QString,QVariant>* ret;
    if (_properties == NULL) {
        ret = new QMap<QString,QVariant>(/*0*/);
    } else {
        ret = new QMap<QString,QVariant> (*_properties/*.clone()*/);
    }
    return new QMap<QString,QVariant> /*Collections.unmodifiableMap*/(*ret);
}
 ...
 * @since 1.5
 */
/*public*/ QVariant AudioFormat::getProperty(QString key) {
    if (_properties == NULL) {
     return QVariant();
    }
 ...
 */
/*public*/ bool AudioFormat::matches(AudioFormat* format) {
 ...
 */
/*public*/ QString AudioFormat::toString() {
    QString sEncoding = "";
    if (getEncoding() != NULL) {
    ...
    if (getSampleRate() == (float) AudioSystem::NOT_SPECIFIED) {
        ...
        sSampleRate =QString::number(getSampleRate()) + " Hz, ";
    }

    QString sSampleSizeInBits;
    if (getSampleSizeInBits() == (float) AudioSystem::NOT_SPECIFIED) {
        sSampleSizeInBits = "unknown bits per sample, ";
    } else {
        sSampleSizeInBits = QString::number(getSampleSizeInBits()) + " bit, ";
    }
    ...
    if (getChannels() == 1) {
        ...
        if (getChannels() == 2) {
            ...
            if (getChannels() == AudioSystem::NOT_SPECIFIED) {
                ...
                sChannels = QString::number(getChannels())+" channels, ";
            }
        }
    ...
    if (getFrameSize() == (float) AudioSystem::NOT_SPECIFIED) {
        ...
        sFrameSize = QString::number(getFrameSize())+ " bytes/frame, ";
    }

    QString sFrameRate = "";
    if (qAbs(getSampleRate() - getFrameRate()) > 0.00001) {
        if (getFrameRate() == (float) AudioSystem::NOT_SPECIFIED) {
            sFrameRate = "unknown frame rate, ";
        } else {
            sFrameRate = QString::number(getFrameRate()) + " frames/second, ";
        }
    }
...
/*public*/ QAudioFormat AudioFormat::qAudioFormat()
{
 _qAudioFormat.setByteOrder(this->bigEndian?QAudioFormat::BigEndian:QAudioFormat::LittleEndian);
 _qAudioFormat.setChannelCount(this->channels);
 _qAudioFormat.setCodec("audio/pcm");
 _qAudioFormat.setSampleRate(this->sampleRate);
 _qAudioFormat.setSampleSize(this->sampleSizeInBits);
 _qAudioFormat.setSampleType(this->sampleSizeInBits>8?QAudioFormat::SignedInt:QAudioFormat::UnSignedInt); // Unknown, SignedInt, UnSignedInt, Float
 return _qAudioFormat;
}
/*public*/ AudioFormat::AudioFormat(QAudioFormat qAudioFormat)
{
 this->encoding = new Encoding("");
 this->sampleRate = qAudioFormat.sampleRate();
 this->sampleSizeInBits = qAudioFormat.sampleSize();
 this->channels = qAudioFormat.channelCount();
// this->frameSize = qAudioFormat.;
// this->frameRate = 0;
 this->bigEndian = qAudioFormat.byteOrder()== QAudioFormat::BigEndian;
 this->_properties = NULL;
...
///*public*/ static class Encoding {
     ...
     */
    /*public*/ /*static*/ /*final*/ Encoding* Encoding::PCM_SIGNED = new Encoding("PCM_SIGNED");

    /**
     * Specifies unsigned, linear PCM data.
     */
    /*public*/ /*static*/ /*final*/ Encoding* Encoding::PCM_UNSIGNED = new Encoding("PCM_UNSIGNED");
     ...
     * @since 1.7
     */
    /*public*/ /*static*/ /*final*/ Encoding* Encoding::PCM_FLOAT = new Encoding("PCM_FLOAT");

    /**
     * Specifies u-law encoded data.
     */
    /*public*/ /*static*/ /*final*/ Encoding* Encoding::ULAW = new Encoding("ULAW");

    /**
     * Specifies a-law encoded data.
     */
    /*public*/ /*static*/ /*final*/ Encoding* Encoding::ALAW = new Encoding("ALAW");
...
//    /*private*/ QString name;
     ...
     */
    /*public*/ Encoding::Encoding(QString name)
 : QObject()
{
        this->name = name;
    }
...
//     */
//    /*public*/ /*final*/ bool Encoding::equals(QObject* obj) {
//        if (toString() == NULL) {
//            return (obj != NULL) && (obj->objectName() == NULL);
//        }
//        //if (obj instanceof Encoding)
//        if(qobject_cast<Encoding*>(obj) != NULL)
//        {
...
//    }
     ...
     */
    /*public*/ /*final*/ int Encoding::hashCode() {
        if (toString() == NULL) {
            return 0;
        }
     ...
     */
    /*public*/ /*final*/ QString Encoding::toString() {
        return name;
    }
