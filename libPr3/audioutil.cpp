...
///*public*/ class AudioUtil {
     ...
     */
    /*static*/ /*private*/ QList<AudioByteBuffer>* AudioUtil::splitInputStream(QDataStream* stream, int max_time, int min_time) {
       QList<AudioByteBuffer>* rlist =  new QList<AudioByteBuffer>();
        QVector<int> format =  QVector<int>(1);
        QVector<QByteArray> data =  QVector<QByteArray>(1);
        QVector<int> size =  QVector<int>(1);
        QVector<int> freq =  QVector<int>(1);
        QVector<int> loop =  QVector<int>(1);
#if 0// TODO:
     ...
     */
    /*static*/ /*public*/ QList<AudioBuffer*>* AudioUtil::getAudioBufferList(QString prefix, QList<AudioByteBuffer>* blist) {
 Logger* log = new Logger("AudioUtil");
        // Sanity check the prefix, since if it's wrong we'll get a casting error below.
        if (prefix.at(2) != Audio::BUFFER) {
            log->warn("Not a Buffer request! " + prefix);
            return NULL;
        }

        QList<AudioBuffer*>* rlist = new QList<AudioBuffer*>();
#if 0 // TODO:
        int i = 0; // Index used for the sub-buffer system names
        foreach (AudioByteBuffer b, *blist) {
            try {
                AudioBuffer* buf = (AudioBuffer*) InstanceManager::audioManagerInstance()->provideAudio(prefix + "_sbuf" + i);
                i++;
                if (buf == NULL ) {
                ...
                if (log->isDebugEnabled()) {
                    InstanceManager::audioManagerInstance().getSystemNameList(Audio.BUFFER).stream().forEach((s) -> {
                        log->debug("\tBuffer: " + s);
                    });
                }
                return NULL ;
    ...
    /*public*/ /*static*/ QList<AudioBuffer*>* AudioUtil::getAudioBufferList(QString prefix, QDataStream* stream, int max_time, int min_time) {
        QList<AudioBuffer*>* rlist = NULL;
        QList<AudioByteBuffer>* blist = splitInputStream(stream, max_time, min_time);
        if (blist != NULL) {
...
//        /*public*/ int loop;
        ...
        /*public*/ AudioByteBuffer::AudioByteBuffer(QByteArray d, int fmt, int freq, int lp) {
            data = d;
            format = fmt;
            frequency = freq;
            loop = lp;
