 ...
 * constructor method
 */
/*public*/ LayoutSlip::LayoutSlip(QString id,  LayoutEditor* layoutEditor, LayoutTurnout::TurnoutType type)
 : LayoutTurnout(id, layoutEditor, type)
{
 init();
 //instance = this;
 this->type = type;
 ...
 */
/*public*/ void LayoutSlip::toggleState(HitPointType::TYPES selectedPointType) {
...
/*public*/ void LayoutSlip::reCheckBlockBoundary() const{
 ...
 if (qobject_cast<TrackSegment*>(connectD)) {
 ...
 */
//@Override
/*public*/ QList<HitPointType::TYPES> LayoutSlip::checkForFreeConnections() {
   QList<HitPointType::TYPES> result =  QList<HitPointType::TYPES>();

    //check the A connection point
    if (getConnectA() == nullptr) {
        result.append((HitPointType::SLIP_A));
    }

    //check the B connection point
    if (getConnectB() == nullptr) {
        result.append((HitPointType::SLIP_B));
    }

    //check the C connection point
    if (getConnectC() == nullptr) {
        result.append((HitPointType::SLIP_C));
    }

    //check the D connection point
    if (getConnectD() == nullptr) {
        result.append((HitPointType::SLIP_D));
    }
    return result;
}
