 ...
 * @since 1.6
 */
/*public*/ /*static*/ /*final*/ int Path2D::WIND_EVEN_ODD = PathIterator::WIND_EVEN_ODD;
 ...
 * @since 1.6
 */
/*public*/ /*static*/ /*final*/ int Path2D::WIND_NON_ZERO = PathIterator::WIND_NON_ZERO;

// For code simplicity, copy these constants to our namespace
// and cast them to byte constants for easy storage.
/*private*/ /*static*/ /*final*/ char Path2D::SEG_MOVETO  = (char) PathIterator::SEG_MOVETO;
/*private*/ /*static*/ /*final*/ char Path2D::SEG_LINETO  = (char) PathIterator::SEG_LINETO;
/*private*/ /*static*/ /*final*/ char Path2D::SEG_QUADTO  = (char) PathIterator::SEG_QUADTO;
/*private*/ /*static*/ /*final*/ char Path2D::SEG_CUBICTO = (char) PathIterator::SEG_CUBICTO;
/*private*/ /*static*/ /*final*/ char Path2D::SEG_CLOSE   = (char) PathIterator::SEG_CLOSE;


/*static*/ /*final*/ int Path2D::INIT_SIZE = 20;
/*static*/ /*final*/ int Path2D::EXPAND_MAX = 500;
/*static*/ /*final*/ int Path2D::EXPAND_MAX_COORDS = Path2D::EXPAND_MAX * 2;
/*static*/ /*final*/ int Path2D::EXPAND_MIN = 10; // ensure > 6 (cubics)
...
Path2D::Path2D(int rule, int initialTypes)
{
 ...
 numCoords = 0;
    setWindingRule(rule);
    this->pointTypes = new QVector<char>(initialTypes);
}

/*abstract*/ QVector<float>* Path2D::cloneCoordsFloat(AffineTransform* at) {return NULL;}
/*abstract*/ QVector<double>* Path2D::cloneCoordsDouble(AffineTransform* at) {return NULL;}
/*abstract*/ void Path2D::append(float x, float y) {}
/*abstract*/ void Path2D::append(double x, double y) {}
/*abstract*/ QPointF Path2D::getPoint(int coordindex) {return QPointF();}
/*abstract*/ void Path2D::needRoom(bool needMove, int newCoords) {}
/*abstract*/ int Path2D::pointCrossings(double px, double py) { return 0;}
/*abstract*/ int Path2D::rectCrossings(double rxmin, double rymin,
                           double rxmax, double rymax) {return 0;}

/*static*/ QVector<char>* Path2D::expandPointTypes(QVector<char>* oldPointTypes, int needed) \
{
    ...
    if (newSizeMin < oldSize) {
        ...
        Logger::error("pointTypes exceeds maximum capacity !");
    }
    // growth algorithm computation
    int grow = oldSize;
    if (grow > EXPAND_MAX) {
        grow = qMax(EXPAND_MAX, oldSize >> 3); // 1/8th min
...
//    transient float floatCoords[];
     ...
     * @since 1.6
     */
    /*public*/ Float::Float() : Path2D(WIND_NON_ZERO, INIT_SIZE)
    {
        //this(WIND_NON_ZERO, INIT_SIZE);
     log = new Logger("Path2D_Float");
     ...
     * @since 1.6
     */
    /*public*/ Float::Float(int rule) : Path2D(rule, INIT_SIZE)
{
        //this(rule, INIT_SIZE);
     log = new Logger("Path2D_Float");
    }
     ...
     * @since 1.6
     */
    /*public*/ Float::Float(int rule, int initialCapacity) : Path2D(rule, initialCapacity)
{
        //super(rule, initialCapacity);
     log = new Logger("Path2D_Float");
        floatCoords = new QVector<float>(initialCapacity * 2);
    }
     ...
     * @since 1.6
     */
//    /*public*/ Path2D_Float::Path2D_Float(JShape* s)
//    {
//        //this(s, NULL);
 ...
 * @since 1.6
 */
/*public*/ Float::Float(JShape* s, AffineTransform* at)
{
 log = new Logger("Path2D_Float");

        //if (s instanceof Path2D) {
 //if(qobject_cast<Path2D*>(s)!= NULL)
 if(QString(s->metaObject()->className()).contains("Path2D"))
 {
  Path2D* p2d = (Path2D*) s;
 ...
 else
 {
  ...
  this->pointTypes = new QVector<char>(INIT_SIZE);
  this->floatCoords = new QVector<float>(INIT_SIZE * 2);
  append(pi, false);
 }
}

    //@Override
    QVector<float>* Float::cloneCoordsFloat(AffineTransform* at) {
        // trim arrays:
        QVector<float>* ret;
        if (at == NULL) {
            ret = copyOf(floatCoords, numCoords);
        } else {
            ret = new QVector<float>(numCoords);
    ...
    //@Override
    QVector<double>* Float::cloneCoordsDouble(AffineTransform* at) {
        // trim arrays:
        QVector<double>* ret = new QVector<double>(numCoords);
        if (at == NULL) {
            for (int i = 0; i < numCoords; i++) {
                ret->replace(i, floatCoords->at(i));
            }
        } else {
            at->transform(floatCoords, 0, ret, 0, numCoords / 2);
        }
    ...
    void Float::append(float x, float y) {
        floatCoords->replace(numCoords++, x);
        floatCoords->replace(numCoords++, y);
    }

    void Float::append(double x, double y) {
        floatCoords->replace(numCoords++, (float) x);
        floatCoords->replace(numCoords++, (float) y);
    }

    QPointF Float::getPoint(int coordindex) {
        return  QPointF(floatCoords->at(coordindex),
                                 floatCoords->at(coordindex+1));
    }

    //@Override
    void Float::needRoom(bool needMove, int newCoords) {
        if ((numTypes == 0) && needMove) {
            log->error(QString("missing initial moveto ")+
                                                "in path definition");
        }
    ...
    /*static*/ QVector<float>* Float::expandCoords(QVector<float>* oldCoords, int needed) {
        /*final*/ int oldSize = oldCoords->length();
        /*final*/ int newSizeMin = oldSize + needed;
        if (newSizeMin < oldSize) {
            // hard overflow failure - we can't even accommodate
            // new items without overflowing
            Logger::error("coords exceeds maximum capacity !");
        }
        // growth algorithm computation
        int grow = oldSize;
        if (grow > EXPAND_MAX_COORDS) {
            grow = qMax(EXPAND_MAX_COORDS, oldSize >> 3); // 1/8th min
        } else if (grow < EXPAND_MIN) {
            grow = EXPAND_MIN;
        }
        Q_ASSERT(grow > needed);
...
//            }
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::moveTo(double x, double y) {
        if (numTypes > 0 && pointTypes->at(numTypes - 1) == SEG_MOVETO) {
            floatCoords->replace(numCoords-2,  (float) x);
            floatCoords->replace(numCoords-1,  (float) y);
        } else {
            needRoom(false, 2);
            pointTypes->replace(numTypes++,  SEG_MOVETO);
            floatCoords->replace(numCoords++,  (float) x);
            floatCoords->replace(numCoords++,  (float) y);
        }
    }
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::moveTo(float x, float y) {
        if (numTypes > 0 && pointTypes->at(numTypes - 1) == SEG_MOVETO) {
            floatCoords->replace(numCoords-2,  x);
            floatCoords->replace(numCoords-1,  y);
        } else {
            needRoom(false, 2);
            pointTypes->replace(numTypes++, SEG_MOVETO);
            floatCoords->replace(numCoords++, x);
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::lineTo(double x, double y) {
        needRoom(true, 2);
        pointTypes->replace(numTypes++, SEG_LINETO);
        floatCoords->replace(numCoords++, (float) x);
        floatCoords->replace(numCoords++, (float) y);
    }
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::lineTo(float x, float y) {
        needRoom(true, 2);
        pointTypes->replace(numTypes++, SEG_LINETO);
        floatCoords->replace(numCoords++, x);
        floatCoords->replace(numCoords++, y);
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::quadTo(double x1, double y1,
                                          double x2, double y2)
    {
        needRoom(true, 4);
        pointTypes->replace(numTypes++, SEG_QUADTO);
        floatCoords->replace(numCoords++, (float) x1);
        floatCoords->replace(numCoords++, (float) y1);
        floatCoords->replace(numCoords++, (float) x2);
        floatCoords->replace(numCoords++, (float) y2);
    }
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::quadTo(float x1, float y1,
                                          float x2, float y2)
    {
        needRoom(true, 4);
        pointTypes->replace(numTypes++, SEG_QUADTO);
        floatCoords->replace(numCoords++, x1);
        floatCoords->replace(numCoords++, y1);
        floatCoords->replace(numCoords++, x2);
        floatCoords->replace(numCoords++, y2);
    }
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::curveTo(double x1, double y1,
                                           double x2, double y2,
                                           double x3, double y3)
    {
        needRoom(true, 6);
        pointTypes->replace(numTypes++, SEG_CUBICTO);
        floatCoords->replace(numCoords++, (float) x1);
        floatCoords->replace(numCoords++, (float) y1);
        floatCoords->replace(numCoords++, (float) x2);
        floatCoords->replace(numCoords++, (float) y2);
        floatCoords->replace(numCoords++, (float) x3);
        floatCoords->replace(numCoords++, (float) y3);
    }
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ /*synchronized*/ void Float::curveTo(float x1, float y1,
                                           float x2, float y2,
                                           float x3, float y3)
    {
        needRoom(true, 6);
        ...
        floatCoords->replace(numCoords++, x2);
        floatCoords->replace(numCoords++, y2);
        floatCoords->replace(numCoords++, x3);
        floatCoords->replace(numCoords++, y3);
    }
#if 0
    int pointCrossings(double px, double py) {
        ...
        QVector<float coords>* = floatCoords;
        curx = movx = coords[0];
        cury = movy = coords[1];
        int crossings = 0;
        int ci = 2;
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ void Float::append(PathIterator* pi, bool connect) {
     QVector<float>* coords = new QVector<float>(6);
        while (!pi->isDone()) {
            ...
            if(pi->currentSegment(coords)== SEG_MOVETO)
            {
                ...
                if (!connect || numTypes < 1 || numCoords < 1) {
                    moveTo(coords->at(0), coords->at(1));
                    break;
                }
                if (pointTypes->at(numTypes - 1) != SEG_CLOSE &&
                    floatCoords->at(numCoords-2) == coords->at(0) &&
                    floatCoords->at(numCoords-1) == coords->at(1))
                {
                    // Collapse out initial moveto/lineto
                    break;
                }
                lineTo(coords->at(0), coords->at(1));
            }
            else if(pi->currentSegment(coords)== SEG_LINETO)
            {
                lineTo(coords->at(0), coords->at(1));
            }
            else if(pi->currentSegment(coords)== SEG_QUADTO)
            {
                quadTo(coords->at(0), coords->at(1),
                       coords->at(2), coords->at(3));
            }
            else if(pi->currentSegment(coords)== SEG_CUBICTO)
            {
                curveTo(coords->at(0), coords->at(1),
                        coords->at(2), coords->at(3),
                        coords->at(4), coords->at(5));
            }
            else if(pi->currentSegment(coords)== SEG_CLOSE)
            {
                closePath();
                break;
            }
     ...
     * @since 1.6
     */
    /*public*/ /*final*/ PathIterator* Float::getPathIterator(AffineTransform* at) {
        if (at == NULL) {
...
//        float floatCoords[];
        ...
        CopyIterator::CopyIterator(Float* p2df) : Iterator(p2df) {
            //super(p2df);
            this->floatCoords = p2df->floatCoords;
        }

        /*public*/ int CopyIterator::currentSegment(QVector<float>* coords) {
            int type = path->pointTypes->at(typeIdx);
            int numCoords = curvecoords.at(type);
            if (numCoords > 0) {
                AffineTransform::arraycopy(floatCoords, pointIdx,
                                 coords, 0, numCoords);
            }
            return type;
        }

        /*public*/ int CopyIterator::currentSegment(QVector<double>* coords) {
            int type = path->pointTypes->at(typeIdx);
            int numCoords = curvecoords.at(type);
            if (numCoords > 0) {
                for (int i = 0; i < numCoords; i++) {
                    coords->replace(i, floatCoords->at(pointIdx + i));
                }
            }
            return type;
        }
...
//        AffineTransform affine;
        ...
        TxIterator::TxIterator(Float* p2df, AffineTransform* at) : Iterator(p2df)
        {
            //super(p2df);
            this->floatCoords = p2df->floatCoords;
            this->affine = at;
        ...
        /*public*/ int TxIterator::currentSegment(QVector<float>* coords) {
            int type = path->pointTypes->at(typeIdx);
            int numCoords = curvecoords.at(type);
            if (numCoords > 0) {
        ...
        /*public*/ int TxIterator::currentSegment(QVector<double>* coords) {
            int type = path->pointTypes->at(typeIdx);
            int numCoords = curvecoords.at(type);
            if (numCoords > 0) {
                affine->transform(floatCoords, pointIdx,
 ...
 * @since 1.6
 */
/*public*/ /*final*/ /*synchronized*/ void Path2D::closePath() {
    if (numTypes == 0 || pointTypes->at(numTypes - 1) != SEG_CLOSE) {
        needRoom(true, 0);
        pointTypes->replace(numTypes++, SEG_CLOSE);
    }
}
#if 0
/**
 ...
 * @since 1.6
 */
/*public*/ /*final*/ /*synchronized*/ int Path2D::getWindingRule() {
    return windingRule;
}
 ...
 * @since 1.6
 */
/*public*/ /*final*/ void Path2D::setWindingRule(int rule) {
    if (rule != WIND_EVEN_ODD && rule != WIND_NON_ZERO) {
        log->error(QString("winding rule must be ")+
                                           "WIND_EVEN_ODD or "+
                                           "WIND_NON_ZERO");
    }
    windingRule = rule;
...
QVector<float>* Path2D::copyOf(QVector<float> *src, int newLength)
{
 QVector<float>* array = new QVector<float>(newLength);
 for(int i = 0; i < qMin(src->size(), newLength); i++)
 {
  array->replace(i, src->at(i));
 }
 return array;
}
QVector<char>* Path2D::copyOf(QVector<char> *src, int newLength)
{
 QVector<char>* array = new QVector<char>(newLength);
 for(int i = 0; i < qMin(src->size(), newLength); i++)
 {
  array->replace(i, src->at(i));
 }
...
//    Path2D path;
    ...
    /*static*/ /*final*/ QVector<int> Iterator::curvecoords =  QVector<int>() << 2 << 2 << 4 << 6 << 0;


    Iterator::Iterator(Path2D* path) {
        this->path = path;
     typeIdx = 0;
     pointIdx = 0;
    }

    /*public*/ int Iterator::getWindingRule() {
        return path->getWindingRule();
    }

    /*public*/ bool Iterator::isDone() {
        return (typeIdx >= path->numTypes);
    }

    /*public*/ void Iterator::next() {
        int type = path->pointTypes->at(typeIdx++);
        pointIdx += curvecoords.at(type);
    }
//};
