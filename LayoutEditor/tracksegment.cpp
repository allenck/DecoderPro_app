...
/*public*/ TrackSegment::TrackSegment(/*@Nonnull*/ QString id,
        /*@CheckForNull*/ LayoutTrack* c1, HitPointType::TYPES t1,
        /*@CheckForNull*/ LayoutTrack* c2, HitPointType::TYPES t2,
        bool main,
        /*@Nonnull*/ LayoutEditor* models) : LayoutTrack(id, models) {
    //super(id, models);
    init(id);

    // validate input
    if ((c1 == nullptr) || (c2 == nullptr)) {
        log->error(tr("Invalid object in TrackSegment constructor call - %1").arg(id));
    }
    ...
    if (HitPointType::isConnectionHitType(t1)) {
        ...
        log->error(tr("Invalid connect type 1 ('%1') in TrackSegment constructor - %2").arg(metaEnum.valueToKey(t1), id));
    }
    if (HitPointType::isConnectionHitType(t2)) {
        connect2 = c2;
        type2 = t2;
    } else {
        log->error(tr("Invalid connect type 2 ('%1') in TrackSegment constructor - %2").arg(metaEnum.valueToKey(t2), id));
    }
...
/*public*/ TrackSegment::TrackSegment(/*@Nonnull*/ QString id,
        /*@CheckForNull*/ QString c1Name, HitPointType::TYPES t1,
        /*@CheckForNull*/ QString c2Name, HitPointType::TYPES t2,
        bool main,
        /*@Nonnull*/ LayoutEditor* models) : LayoutTrack(id, models){
    //super(id, models);
    init(id);
...
/*public*/ HitPointType::TYPES TrackSegment::getType1() {
    return type1;
}

/*public*/ HitPointType::TYPES TrackSegment::getType2() {
    return type2;
}
 ...
 */
/*protected*/ void TrackSegment::setNewConnect1(/*@CheckForNull*/ LayoutTrack* connectTrack, HitPointType::TYPES  connectionType) {
 ...
 * @param connectionType where on that track we want to be connected
 */
/*protected*/ void TrackSegment::setNewConnect2(LayoutTrack *connectTrack, HitPointType::TYPES connectionType) {
 ...
 */
/*public*/ bool TrackSegment::replaceTrackConnection(/*@CheckForNull*/ LayoutTrack* oldTrack, /*@CheckForNull*/ LayoutTrack* newTrack, HitPointType::TYPES newType) {
...
/*private*/ QString TrackSegment::getConnectName(/*@Nullable*/ LayoutTrack* layoutTrack, HitPointType::TYPES /*type*/) {
...
/*public*/ void TrackSegment::setConnect2(/*@CheckForNull*/ LayoutTrack* o, HitPointType::TYPES type) {
    type2 = type;
    connect2 = o;
}
...
/*public*/ void TrackSegment::setObjects(LayoutEditor* p)
{
  ...
  if (!tLayoutBlockName.isEmpty()) {
      ...
      if (lb != nullptr) {
       ...
       log->error(tr("%1.setObjects(...); bad blockname '%2' in tracksegment %3").arg(
               getName(), tLayoutBlockName, getName()));
          namedLayoutBlock = nullptr;
      }
      tLayoutBlockName = QString(); //release this memory
  }
...
/*private*/ LayoutBlock* TrackSegment::getBlock (LayoutTrack* connect, int type)
{
 ...
 {
  return (models->getAffectedBlock(connect,type));
 }
}
 ...
 */
//@Override
/*public*/ bool TrackSegment::canRemove() {
    QList<QString> itemList = QList<QString>();

    HitPointType::TYPES  type1 = getType1();
    LayoutTrack* conn1 = getConnect1();
    //itemList.addAll(getPointReferences(type1, conn1));
    foreach(QString s, getPointReferences(type1, conn1))
    {
     itemList.append(s);
    }

    HitPointType::TYPES type2 = getType2();
    LayoutTrack* conn2 = getConnect2();
    //itemList.addAll(getPointReferences(type2, conn2));
    foreach(QString s, getPointReferences(type2, conn2))
    {
     itemList.append(s);
    }
...
/*public*/ QList<QString> TrackSegment::getPointReferences(HitPointType::TYPES type, LayoutTrack* conn) {
    ...
    if (type == HitPointType::POS_POINT && qobject_cast<PositionablePoint*>(conn)) {
        ...
        if (pt->getType() == PositionablePoint::PointType::EDGE_CONNECTOR && pt->getLinkedPoint() != nullptr) {
            result.append(tr("Active edge connector link"));   // NOI18N
        }
    }
    ...
    if (HitPointType::isTurnoutHitType(type) && qobject_cast<LayoutTurnout*>(conn)) {
        ...
        switch (type) {
            ...
            default: {
         QMetaEnum metaEnum = QMetaEnum::fromType<HitPointType::TYPES>();
                log->error(tr("Unexpected HitPointType: %1").arg(metaEnum.valueToKey(type)));
            }
        }
    }
...
//{
//  QPointF pt1 = LayoutEditor::getCoords(getConnect1(),getType1().type);
 ...
 if (lb1 != nullptr)
 {
  ...
  if (HitPointType::isTurnoutHitType(type2))
  {
   ...
   if(getConnect2() == nullptr)  //ACK temp
    ...
    if (lt->getTurnoutType() > LayoutTurnout::WYE_TURNOUT) {
        // not RH, LH, or WYE turnout - other blocks possible
        if ((type2 == HitPointType::TURNOUT_B) && (lt->getLayoutBlockB() != nullptr)) {
            lb2 = lt->getLayoutBlockB();
        }
        if ((type2 == HitPointType::TURNOUT_C) && (lt->getLayoutBlockC() != nullptr)) {
            lb2 = lt->getLayoutBlockC();
        }
        if ((type2 == HitPointType::TURNOUT_D) && (lt->getLayoutBlockD() != nullptr)) {
            lb2 = lt->getLayoutBlockD();
        }
    }
    if ((lb2 != nullptr) && (lb1 != lb2)) {
 ...
 */
/*//@Override
public*/ QList<HitPointType::TYPES> TrackSegment::checkForFreeConnections() {
