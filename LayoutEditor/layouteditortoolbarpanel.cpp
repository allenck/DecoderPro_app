 ...
 : JPanel(parent)
{
 ...
 blockIDComboBox = new NamedBeanComboBox((AbstractManager*)InstanceManager::getDefault("BlockManager"), nullptr, NamedBean::DisplayOptions::DISPLAYNAME);
 highlightBlockCheckBox = new QCheckBox(tr("Highlight"));

 blockSensorLabel = new JLabel(tr("%1").arg(tr("Block Sensor Name")));
 blockSensorComboBox = new NamedBeanComboBox((AbstractProxyManager*) InstanceManager::getDefault("SensorManager"), nullptr, NamedBean::DisplayOptions::DISPLAYNAME);
 ...
 signalMastComboBox = new NamedBeanComboBox((DefaultSignalMastManager*)InstanceManager::getDefault("SignalMastManager"), nullptr, NamedBean::DisplayOptions::DISPLAYNAME);
 ...
 signalHeadComboBox = new NamedBeanComboBox((AbstractManager*)InstanceManager::getDefault("SignalHeadManager"), nullptr, NamedBean::DisplayOptions::DISPLAYNAME);
...
/*public*/ LayoutEditorToolBarPanel::LocationFormat LayoutEditorToolBarPanel::getLocationFormat() {
   return locationFormat;
}

/*public*/ void LayoutEditorToolBarPanel::setLocationFormat(LocationFormat locationFormat) {
   if (this->locationFormat != locationFormat) {
       switch (locationFormat) {
           default:
           case ePIXELS: {
               QSize coordSize =  JLabel("10000").sizeHint();
               xLabel->resize(coordSize);
               yLabel->resize(coordSize);
               break;
           }
           case eMETRIC_CM: {
               QSize coordSize =  JLabel(getMetricCMText(10005)).sizeHint();
               xLabel->resize(coordSize);
               yLabel->resize(coordSize);
           ...
           case eENGLISH_FEET_INCHES: {
               QSize coordSize = JLabel(getEnglishFeetInchesText(100008)).sizeHint();
               xLabel->resize(coordSize);
               yLabel->resize(coordSize);
       ...
       if(prefsMgr)
       {
           QString windowFrameRef = layoutEditor->getWindowFrameRef();
           QMetaEnum me = QMetaEnum::fromType<LocationFormat>();
           prefsMgr->setProperty(windowFrameRef, "LocationFormat", /*locationFormat.name()*/QString(me.valueToKey(locationFormat)));
       }//);
       setLocationText(lastLocation);
   }
}

/*public*/ void LayoutEditorToolBarPanel::setLocationText(QPointF p) {
   int x = (int) p.x();
   int y = (int) p.y();

   // default behaviour is pixels
   QString xText = QString::number(x);
   QString yText = QString::number(y);

   if (locationFormat == (LocationFormat::eENGLISH_FEET_INCHES)) {
       xText = getEnglishFeetInchesText(x);
       yText = getEnglishFeetInchesText(y);
   } else if (locationFormat == (LocationFormat::eMETRIC_CM)) {
...
/*private*/ QString LayoutEditorToolBarPanel::getEnglishFeetInchesText(int v) {
   QString result = "";
   ...
   if (feet > 0) {
       result = tr("%1'").arg(feet);
   }

   bool inchesFlag = false;
   if ((v == 0) || (inches > 0)) {
       result += tr(" %1").arg(inches);
       inchesFlag = true;
   }

   if (numer > 0) {
       result += tr(" %1/%2").arg(numer, denom);
       inchesFlag = true;
   }
   if (inchesFlag) {
       result += "\"";
...
/*private*/ QString LayoutEditorToolBarPanel::getMetricCMText(int v) {
   return tr("%1.%2 cm").arg(v / 10, v % 10);
}
