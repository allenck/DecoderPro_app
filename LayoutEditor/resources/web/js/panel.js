...
//draw a SlipTurnout (pass in widget)
...
//		if (window.console) console.log("could not get trackwidth of "+$widget.connectaname+" for "+$widget.name);
    ...
    var cenx = $widget.xcen;
    var ceny = $widget.ycen
    var ax = $gPts[$widget.ident + SLIP_A].x;  //retrieve the points
...
//    $drawLine(dx, dy, bx, by, $color, $width); //D to B
    ...
    var ad13x = (ax * 1) + ((dx - ax) * 0.33); // A to D
    var ad13y = (ay * 1) + ((dy - ay) * 0.33);
    var ad23x = (ax * 1) + ((dx - ax) * 0.66);
    var ad23y = (ay * 1) + ((dy - ay) * 0.66);
    var ac13x = (ax * 1) + ((cx - ax) * 0.33); // A to C
    var ac13y = (ay * 1) + ((cy - ay) * 0.33);
    var ac23x = (ax * 1) + ((cx - ax) * 0.66);
    var ac23y = (ay * 1) + ((cy - ay) * 0.66);

    var bc13x = (bx * 1) + ((cx - bx) * 0.33); // B to C
    var bc13y = (by * 1) + ((cy - by) * 0.33);
    var bc23x = (bx * 1) + ((cx - bx) * 0.66);
    var bc23y = (by * 1) + ((cy - by) * 0.66);
    var bd13x = (bx * 1) + ((dx - bx) * 0.33); // B to D
    var bd13y = (by * 1) + ((dy - by) * 0.33);
    var bd23x = (bx * 1) + ((dx - bx) * 0.66);
    var bd23y = (by * 1) + ((dy - by) * 0.66);

    $drawLine(ax, ay, ad13x, ad13y, $color, $width); //A to D
    $drawLine(ad23x, ad23y, dx, dy, $color, $width);
    $drawLine(ax, ay, ac13x, ac13y, $color, $width); //A to C
    $drawLine(ac23x, ac23y, cx, cy, $color, $width);
    $drawLine(bx, by, bd13x, bd13y, $color, $width); //B to D
    $drawLine(bd23x, bd23y, dx, dy, $color, $width);
    $drawLine(bx, by, bc13x, bc13y, $color, $width); //B to C
    $drawLine(bc23x, bc23y, cx, cy, $color, $width);

    if (window.console)
        console.log("Info: state for slip " + $widget.id + " state = "
+ $widget.state);
}
...
//		if (window.console) console.log("could not get trackwidth of "+$widget.connectaname+" for "+$widget.name);
    ...
    if ($widget.type == LH_TURNOUT || $widget.type == RH_TURNOUT || $widget.type == WYE_TURNOUT) {
        ...
        if ($widget.state == CLOSED || $widget.state == THROWN) {
        ...
        } else { // added to show undefined state
            var a13x = (ax * 1) + ((bx - ax) * 0.33); // A to B
            var a13y = (ay * 1) + ((by - ay) * 0.33);
            var a23x = (ax * 1) + ((bx - ax) * 0.66);
            var a23y = (ay * 1) + ((by - ay) * 0.66);

            var d13x = (dx * 1) + ((cx - dx) * 0.33); // D to C
            var d13y = (dy * 1) + ((cy - dy) * 0.33);
            var d23x = (dx * 1) + ((cx - dx) * 0.66);
            var d23y = (dy * 1) + ((cy - dy) * 0.66);

            $drawLine(ax, ay, a13x, a13y, $color, $width); //A to B
            $drawLine(a23x, a23y, bx, by, $color, $width);
            $drawLine(dx, dy, d13x, d13y, $color, $width); //D to C
            $drawLine(d23x, d23y, cx, cy, $color, $width);
            ...
            var db13x = (dx * 1) + ((bx - dx) * 0.33); // D to B
            var db13y = (dy * 1) + ((by - dy) * 0.33);
            var db23x = (dx * 1) + ((bx - dx) * 0.66);
            var db23y = (dy * 1) + ((by - dy) * 0.66);

            var $swidth = $gPanel.sidetrackwidth;
            $drawLine(ax, ay, ac13x, ac13y, $color, $swidth); //A to C
            $drawLine(ac23x, ac23y, cx, cy, $color, $swidth);
            $drawLine(dx, dy, db13x, db13y, $color, $swidth); //D to B
            $drawLine(db23x, db23y, bx, by, $color, $swidth);
...
//store the various points defined with a layoutSlip (pass in widget)
...
function $storeSlipPoints($widget) {
    ...
    $gPts[$t.ident] = $t;
    $t = [];
    $t['ident'] = $widget.ident + SLIP_C;  //calculate and store A endpoint (mirror of A for these)
    $t['x'] = $widget.xcen - ($widget.xa - $widget.xcen);
    $t['y'] = $widget.ycen - ($widget.ya - $widget.ycen);
    $gPts[$t.ident] = $t;
    $t = [];
