...
namespace TimeTable
{
 TimeTableGraphCommon::TimeTableGraphCommon(QObject *parent) : QObject(parent)
 {
 ...
 // /*public*/ class TimeTableGraphCommon {
     ...
     */
    void TimeTableGraphCommon::init(int segmentId, int scheduleId, bool showTrainTimes, double height, double width, bool displayType) {
    ...
    /*final*/ /*static*/ QPen TimeTableGraphCommon::gridstroke =  QPen(QBrush(Qt::black),0.5f);
    /*final*/ /*static*/ QPen TimeTableGraphCommon::stroke =  QPen(QBrush(Qt::black),2.0f);


    /*public*/ void TimeTableGraphCommon::doPaint(QPainter* g) {
     _g2 = g;
#if 0
        if (g instanceof EditScene* g) {
            _g2 = (Graphics2D) g;
        } else {
            throw new IllegalArgumentException();
        }
#endif
        //_g2->setRenderHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        _g2->setRenderHint(QPainter::Antialiasing, true);
...
//         double dimWidth = _dimWidth;
    ...
    void TimeTableGraphCommon::drawInfoSection() {
        // Info section
        _g2->setFont(_stdFont);
        //_g2->setColor(Color.BLACK);
        _g2->setPen(Qt::black);
        QString layoutName = tr("%1 %2").arg(tr("Layout Name:")).arg(_layout->getLayoutName());  // NOI18N
        QString segmentName = tr("%1 %2").arg(tr("SegmentName:")).arg(_segment->getSegmentName());  // NOI18N
        QString scheduleName = tr("%1 %2").arg(tr("Schedule Name")).arg(_schedule->getScheduleName());  // NOI18N
        QString effDate = tr("%1 %2").arg(tr("Effective Date:")).arg(_schedule->getEffDate());  // NOI18N

        _infoColWidth = qMax(_infoColWidth, _g2->fontMetrics().width(layoutName));
        _infoColWidth = qMax(_infoColWidth, _g2->fontMetrics().width(scheduleName));
        _infoColWidth = qMax(_infoColWidth, _g2->fontMetrics().width(effDate));

        _g2->drawText(QPoint(10, 20),layoutName);
        _g2->drawText(QPoint(10, 40), segmentName );
        _g2->drawText(QPoint(10, 60), scheduleName);
        _g2->drawText(QPoint(10, 80),effDate);
    }

    void TimeTableGraphCommon::drawStationSection() {
        _maxDistance = _stations.value(_stations.size() - 1)->getDistance();
        _g2->setFont(_stdFont);
        //_g2->setColor(Color.BLACK);
        _g2->setPen(Qt::black);
        _stationGrid.clear();
        for (Station* station : _stations) {
            ...
            _infoColWidth = qMax(_infoColWidth, _g2->fontMetrics().width(stationName) + 5);
            ...
            _g2->drawText(QPointF(15.0f, (float) stationY), stationName );
            _stationGrid.insert(station->getStationId(), stationY);
        }
    }

    void TimeTableGraphCommon::drawHours() {
        ...
        for (int i = 0; i <= _duration; i++) {
            QString hourString = QString::number(currentHour);
            double hourX = (hourWidth * i) + _hourOffset + _graphLeft;
            int hOffset = _g2->fontMetrics().width(hourString) / 2;
            _g2->drawText(QPointF((float) hourX - hOffset, (float) _graphBottom + 20), hourString);
            if (i < _duration) {
                _hourMap.insert(currentHour, hourX);
            }
            _hourGrid.append(hourX);
            if (i == 0) {
    ...
    void TimeTableGraphCommon::drawThrottleNumbers() {
        _g2->setFont(_smallFont);
        //_g2->setColor(Color.BLACK);
        _g2->setPen(Qt::black);
        for (int i = 1; i <= _throttles; i++) {
            _g2->drawText(QPointF((float) _graphLeft, (float) i * 14), QString::number(i) );
        }
    }

    void TimeTableGraphCommon::drawGraphGrid() {
        // Print the graph box
        _g2->drawRect(_graphLeft, _graphTop, _graphWidth, _graphHeight);

        // Print the grid lines
        //_g2->setStroke(gridstroke);
        //_g2->setColor(Color.GRAY);
        gridstroke.setColor(Qt::gray);
        _g2->setPen(gridstroke);
        //_stationGrid.forEach((i, y) ->
        foreach(double y, _stationGrid.values())
        {
            _g2->drawLine(_graphLeft, y, _graphRight, y);
        }//);
        //_hourGrid.forEach((x) ->
        foreach(double x, _hourGrid)
        {
            _g2->drawLine(x, _graphTop, x, _graphBottom);
        }//);
    }
     ...
     */
    void TimeTableGraphCommon::drawTrains() {
//         _baseTime = _startHour * 60;
        _sizeMinute = _graphWidth / ((_duration + 1) * 60);
        _throttleX = 0;
        for (TTTrain* train : _trains) {
            _trainName = train->getTrainName();
            _trainThrottle = train->getThrottle();
            QString typeColor = _dataMgr->getTrainType(train->getTypeId())->getTypeColor();
            _trainColor = QColor(typeColor);
            _trainLine = new QPainterPath();
            ...
            for (_stopIdx = 0; _stopIdx < _stopCnt; _stopIdx++) {
                Stop* stop = _stops.value(_stopIdx);
     ...
     */
    void TimeTableGraphCommon::drawTrainName(double x, double y, QString justify, bool invert, bool throttle) {
        QRectF textRect = _g2->fontMetrics().tightBoundingRect(_trainName);

        // Position train name
        if (justify == ("Center")) {  // NOI18N
            x = x - textRect.width() / 2;
        } else if (justify == ("Right")) {  // NOI18N
            x = x - textRect.width();
        }

        if (invert) {
            y = y + ((_direction == ("down") || throttle) ? -7 : 13);  // NOI18N
        } else {
            y = y + ((_direction == ("down") || throttle) ? 13 : -7);  // NOI18N
        }
        ...
        _g2->setFont(_stdFont);
        _g2->setPen(Qt::black);
        _g2->drawText(QPointF(), _trainName, (float) x, (float) y);
        _textLocation.append(textRect);
    }
     ...
     */
    void TimeTableGraphCommon::drawTrainTime(int time, QString mode, double x, double y) {
        if (!_showTrainTimes) {
            return;
        }
        QString minutes = tr("%02d").arg(time % 60);  // NOI18N
        QRectF textRect = _g2->fontMetrics().tightBoundingRect(minutes);
        if(mode == "begin")  // NOI18N
        {
                x = x + ((_direction == ("down")) ? 2 : 2);  // NOI18N
                y = y + ((_direction == ("down")) ? 10 : -1);  // NOI18N
        }
        else if(mode == "arrive")  // NOI18N
        {
                x = x + ((_direction == ("down")) ? 2 : 3);  // NOI18N
                y = y + ((_direction == ("down")) ? -2 : 10);  // NOI18N
        }
        else if(mode ==  "depart")  // NOI18N
        {
                x = x + ((_direction == ("down")) ? 2 : 2);  // NOI18N
                y = y + ((_direction == ("down")) ? 10 : -2);  // NOI18N
        }
        else if(mode ==  "end")  // NOI18N
        {
                x = x + ((_direction == ("down")) ? 0 : 0);  // NOI18N
                y = y + ((_direction == ("down")) ? 0 : 0);  // NOI18N
        }
        else
        {
                log->error(tr("drawTrainTime mode %1 is unknown").arg(mode));  // NOI18N
                return;
        }
        ...
        _g2->setFont(_smallFont);
        _g2->setPen(Qt::gray);
        _g2->drawText(QPointF(), minutes, (float) x, (float) y);
        _textLocation.append(textRect);
    }  // TODO End?
     ...
     */
    QRectF TimeTableGraphCommon::adjustText(QRectF textRect) {
        double xLoc = textRect.x();
        double yLoc = textRect.y();
        double xLen = textRect.width();
        ...
        for (QRectF workRect : _textLocation) {
            if (workRect.y() == yLoc) {
                xMin = workRect.x();
                xMax = xMin + workRect.width();
     ...
     */
    void TimeTableGraphCommon::setDirection() {
        if (_stopCnt == 1) {
        ...
        if (_firstStop) {
            ...
            nextStation = _dataMgr->getStation(_stops.value(_stopIdx + 1)->getStationId());
            _direction = (nextStation->getDistance() > currDistance) ? "down" : "up";  // NOI18N
            return;
        }

        prevStation = _dataMgr->getStation(_stops.value(_stopIdx - 1)->getStationId());
        if (_lastStop) {
        ...
        if (prevStation->getSegmentId() != _segmentId) {
            ...
            QString prevName = prevStation->getStationName();
     ...
     */
    void TimeTableGraphCommon::setBegin(Stop* stop) {
        double x;
        double y;
        ...
        if (_stopIdx > 0) {
            ...
            segmentChange = true;
            Stop* prevStop = _stops.value(_stopIdx - 1);
            ...
            for (Station* segStation : _stations) {
                if (segStation->getStationName() == (prevName)) {
                    ...
                    y = _stationGrid.value(segStation->getStationId());
        ...
        y = _stationGrid.value(stop->getStationId());
     ...
     * @param stop The current stop.
     */
    void TimeTableGraphCommon::drawLine(Stop* stop) {
        double x = calculateX(_arriveTime);
        double y = _stationGrid.value(stop->getStationId());
        _trainLine->lineTo(x, y);
        ...
        if (stop->getDuration() > 0) {
            ...
            if (x < _trainLine->currentPosition().x()) {
     ...
     * @param stop The current stop.
     */
    void TimeTableGraphCommon::setEnd(Stop *stop, bool endSegment) {
        double x;
        double y;
        bool skipLine = false;

        if (_stops.size() == 1 || endSegment) {
            x = _trainLine->currentPosition().x();
            y = _trainLine->currentPosition().y();
            skipLine = true;
        } else {
...
//        _g2->setStroke(stroke);
        stroke.setColor(_trainColor);
        _g2->setPen(stroke);
        if (!skipLine) {
            _trainLine->lineTo(x, y);
        }
        _g2->drawPath(*_trainLine);
        ...
        if (_trainThrottle > 0) {
            ...
            if (x < _throttleX) {
                 _g2->drawLine(_throttleX, throttleY, _graphRight - _hourOffset, throttleY);
                 _g2->drawLine(_graphLeft + _hourOffset, throttleY, x, throttleY);
                ...
                _g2->drawLine(_throttleX, throttleY, x, throttleY);
                drawTrainName(_throttleX + 10, throttleY + 5, "Left", true, true);  // NOI18N
            }
        }
    }
     ...
     * @return the x value.
     */
    double TimeTableGraphCommon::calculateX(int time) {
        if (time < 0) time = 0;
        ...
        return _hourMap.value(hour) + (min * _sizeMinute);
    }

    /*private*/ /*final*/ /*static*/ Logger* TimeTableGraphCommon::log = LoggerFactory::getLogger("TimeTableGraphCommon");
