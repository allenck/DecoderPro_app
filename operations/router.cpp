...
///*public*/ class Router extends TrainCommon {
    ...
    /*private*/ bool Router::checkForSingleTrain(Car* car, Car* _clone) {
        ...
        if (car->getTrack()->getTrackType()==(Track::STAGING) && _train != NULL && !trainServicesCar) {
            ...
            testTrain = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(_clone, _train, _buildReport);
        }
    ...
    /*private*/ bool Router::setCarDestinationTwoTrains(Car* car, QString trackType) {
        ...
        foreach (Track* track, tracks) {
            ...
            Train* secondTrain = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(testCar, _buildReport);
            if (secondTrain == NULL) {
                if (debugFlag) {
            ...
            if (specific==(YES)) {
                ...
                firstTrain = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(testCar, _buildReport);
            }
    ...
    /*private*/ bool Router::setCarDestinationMultipleTrains(Car* car, bool useStaging) {
        ...
        foreach (Track* nlt, _nextLocationTracks) {
            ...
            foreach (Track* llt, _lastLocationTracks) {
                ...
                Train* middleTrain = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(testCar, NULL); // don't add to report
                if (middleTrain != NULL) {
         ...
         if(!loopOk)
          break;
            otherloop: foreach (Track* mlt, _otherLocationTracks) {
                ...
                // does a train service these two locations?
                Train* middleTrain2 = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(testCar, NULL); // don't add to report
                if (middleTrain2 != NULL) {
                    ...
                    foreach (Track* llt, _lastLocationTracks) {
                        ...
                        testCar->setDestinationTrack(llt); // set car to this destination and track
                        Train* middleTrain3 = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(testCar, NULL); // don't add to
                        // report
                    ...
                    foreach (Track* mlt2, _otherLocationTracks) {
                        ...
                        if (middleTrain3 != NULL) {
                            ...
                            foreach (Track* llt, _lastLocationTracks) {
                                ...
                                testCar->setDestinationTrack(llt); // set car to this destination and track
                                // does a train service these two locations?
                                Train* middleTrain4 = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(testCar, NULL); // don't add
                                // to report
    ...
    /*private*/ void Router::loadTracks(Car* car, Car* testCar, QList<Track*> tracks)
    {
        foreach (Track* track, tracks)
        {
         ...
         if (specific==(YES) || specific==(NOT_NOW)) {
             ...
             train = ((TrainManager*)InstanceManager::getDefault("Operations::TrainManager"))->getTrainForCar(testCar, NULL); // don't add to report
         }
