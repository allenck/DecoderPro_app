 ...
 */
/*protected*/ Section* AutoAllocate::autoNextSectionChoice(QList<Section*> sList, AllocationRequest* ar) {
    ...
    for (int i = 0; i < sList.size(); i++) {
        if (at->getEndBlockSection()->getSystemName() == (sList.at(i)->getSystemName())) {
            return sList.at(i);
        }
    }
...
#if 1
/*private*/ bool AutoAllocate::willAllocatingFollowPlan(AllocationRequest* ar, AllocationPlan* ap) {
...
/*private*/ bool AutoAllocate::allocateIfLessThanThreeAhead(AllocationRequest* ar) {
    ...
    if (aSectionList.size() >= 4) {
        ...
        if ((curAS != NULL) && (curAS->getSection()->getOccupancy() != Section::OCCUPIED)) {
    ...
    log->debug(tr("%1: auto allocating Section %2").arg(ar->getActiveTrain()->getTrainName(),
            ar->getSectionName()));
    _dispatcher->allocateSection(ar, NULL);
    return true;
}

/*private*/ bool AutoAllocate::checkForXingPlan(AllocationRequest* ar, ActiveTrain* nt,
...
/*private*/ bool AutoAllocate::checkForPassingPlan(AllocationRequest* ar, ActiveTrain* nt,
        QList<ActiveTrain*> neededByTrainList) {
    // returns 'true' if an AllocationPlan has been set up, returns 'false' otherwise
    Section* nSec = NULL;
    Section* aSec = NULL;
...
/*private*/ bool AutoAllocate::isThereConflictingPlan(ActiveTrain* at, Section* aSec, int aSecSeq,
...
/*private*/ Section* AutoAllocate::getBestOtherSection(QList<Section*> sList, Section* aSec) {
...
/*private*/ int AutoAllocate::findPassingSection(ActiveTrain* at, int aSeq) {
...
#if 1
/*private*/ bool AutoAllocate::sectionNeeded(AllocationRequest* ar, ActiveTrain* at) {
    ...
    if (((DispatcherFrame*)InstanceManager::getDefault("DispatcherFrame"))->getSignalType() == DispatcherFrame::SIGNALMAST) {
        if (!at->isAllocationReversed()) {
            for (int i = 0; i < tsList.size(); i++) {
                if (tsList.at(i)->getSequenceNumber() > curSeq) {
                    for (int j = 0; j < aSectionList.size(); j++) {
...
/*private*/ bool AutoAllocate::sameDirection(AllocationRequest* ar, ActiveTrain* at) {
...
/*private*/ bool AutoAllocate::firstTrainLeadsSecond(ActiveTrain* at, ActiveTrain* nt) {
...
/*private*/ bool AutoAllocate::willTrainsCross(ActiveTrain* at, ActiveTrain* nt) {
...
/*private*/ bool AutoAllocate::areTrainsAdjacent(ActiveTrain* at, ActiveTrain* nt) {
...
/*private*/ int AutoAllocate::getCurrentSequenceNumber(ActiveTrain* at) {
...
/*private*/ Section* AutoAllocate::getCurSection() {
    return curSection;
}

/*private*/ bool AutoAllocate::isSectionAllocatedToTrain(Section* s, int seq, ActiveTrain* at) {
    if ((s == NULL) || (at == NULL)) {
...
/*private*/ bool AutoAllocate::waitingForStartTime(AllocationRequest* ar) {
    if (ar != NULL) {
        ActiveTrain* at = ar->getActiveTrain();
        if (at == NULL) {
            return false;
...
/*private*/ bool AutoAllocate::willLevelXingsBlockTrain(ActiveTrain* at) {
...
/*private*/ bool AutoAllocate::containsLevelXing(Section* s) {
    ...
    QVector<LevelXing*>* temLevelXingList = NULL;
    QVector<Block*>* blockList = s->getBlockList();
    for (int i = 0; i < blockList->size(); i++) {
        ...
        if (temLevelXingList->size() > 0) {
            for (int j = 0; j < temLevelXingList->size(); j++) {
                _levelXingList.append(temLevelXingList->at(j));
            }
        }
    }
    if (_levelXingList.size() > 0) {
